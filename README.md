...
Docker Compose files (`docker-compose.yml`, `carditrackapi-docker-compose.yml`) are available for containerised local runs. Their published ports bind to the host loopback (`127.0.0.1`) only — services talk to each other by Compose service name, so nothing is exposed to the LAN. See [Local development network exposure](docs/infrastructure.md#local-development-network-exposure).
...
All infrastructure runs on **Google Cloud** (project `carditrack-490120`, region `europe-west2`): Cloud Run services (api, web, worker, webhook-receiver per environment, plus the shared `carditrack-common-medgemma` GPU service in `europe-west1`) and Cloud Run Jobs (the migrator and the four AI-pipeline jobs — digest, aggregator, assessor, chat themer), Cloud SQL PostgreSQL 16, Secret Manager, GCS, Pub/Sub (both environments), and an optional domain-gated Load Balancer with Cloud Armor.

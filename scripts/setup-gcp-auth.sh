...
# workflow_ref/job_workflow_ref let a service account be bound to one workflow
# file rather than to the whole repository. Not consumed by anything today —
# every account here binds repository-wide — but mapped in case a future
# identity needs the narrower scope, so adding one is an IAM-binding change
# only, not a provider change too.
...
# carditrack-deploy also posts the digest to Slack (post-digest.yml) — no
# separate identity for that. It already holds project-level
# roles/secretmanager.admin, so the per-secret accessor grant in
# infrastructure/common/secret_manager.tf adds nothing it didn't already have;
# see "Accepted tradeoff" in SETUP.md for what that costs.

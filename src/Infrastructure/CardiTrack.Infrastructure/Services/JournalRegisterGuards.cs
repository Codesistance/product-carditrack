...
internal static partial class JournalRegisterGuards
{
    ...
    [
        ...
        "diagnos", "afib", "fibrillation", "arrhythmi", "atrial", "apnoea", "apnea",
    ...
    /// <summary>
    /// The entries of <see cref="TermsNeedingAGloss"/> that are word stems rather than whole
    /// words, matched by their start alone. Everything else is a whole word — "rem" must not
    /// fire inside "remained", and it used to be listed with a trailing space to say so, which
    /// let "REM." at the end of a sentence through.
    /// </summary>
    private static readonly HashSet<string> Stems = ["arrhythmi"];
    ...
    /// phrase rather than inside it. <c>arrhythmi</c> has no entry: it is a condition — the same
    /// stem sits in <see cref="ConditionMarkers"/>, so <see cref="NamesACondition"/> has already
    /// refused the reply, in any of the word's forms, by the time this runs.
    /// </summary>
    /// <remarks>
    ...
    /// </para>
    /// <para>
    /// Which terms are bare is decided against the reply as the model wrote it, all at once,
    /// before anything is inserted: the guard treats a marker anywhere in the sentence as the
    /// explanation, so the bracket written in for one term would otherwise make a second bare
    /// term in the same sentence look explained. A term whose first use lies inside a phrase
    /// already glossed ("rem" inside "rem sleep") is skipped, or the phrase would be explained
    /// twice.
    /// </para>
    /// </remarks>
    internal static (string Text, IReadOnlyList<string> Glossed) Gloss(string text)
    {
        ...
        var insertions = new List<(int At, string Gloss)>();
        ...
        foreach (var (term, gloss) in Glosses)
        {
            ...
            var match = TermPattern(term).Match(text);
            if (!match.Success)
                continue;

            var end = match.Index + match.Length;
            if (spans.Any(s => match.Index < s.End && end > s.Start))
                continue;

            spans.Add((match.Index, end));
            insertions.Add((end, $" ({gloss})"));
            glossed.Add(term);
        }

        // Back to front, so each insertion leaves the positions before it where they were.
        var result = new StringBuilder(text);
        foreach (var (at, gloss) in insertions.OrderByDescending(i => i.At))
            result.Insert(at, gloss);

        return (result.ToString(), glossed);
    }
    ...
    /// <summary>
    /// <paramref name="term"/> as a whole word or phrase in the reply's own casing and line
    /// breaks: bounded by anything that is not a letter or digit (a plain <c>\b</c> would not
    /// close after the subscript two in "SpO₂"), with a run of whitespace allowed wherever the
    /// term has a space. A <see cref="Stems"/> entry is bounded at its start only.
    /// </summary>
    private static Regex TermPattern(string term) =>
        Patterns.GetOrAdd(term, static t => new Regex(
            @"(?<![\p{L}\p{N}])" + Regex.Escape(t).Replace(@"\ ", @"\s+")
                + (Stems.Contains(t) ? string.Empty : @"(?![\p{L}\p{N}])"),

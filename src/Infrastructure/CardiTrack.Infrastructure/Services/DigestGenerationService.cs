...
public partial class DigestGenerationService : IDigestGenerationService
{
    ...
    /// </summary>
    private static readonly TimeOnly DayReviewEarliestLocalTime = new(2, 0);

    public async Task<int> GenerateDueDayReviewsAsync(DateTime utcNow, CancellationToken ct = default)
    {
        ...
        foreach (var memberId in memberIds)
        {
            ...
            try
            {
                if (await GenerateDayReviewForMemberAsync(memberId, utcNow, ct))
                    generated++;
            }
            catch (Exception ex)
            {
    ...
    /// </para>
    /// </remarks>
    private async Task<bool> GenerateDayReviewForMemberAsync(
        Guid memberId, DateTime utcNow, CancellationToken ct)
    {
        ...
        if (TimeOnly.FromDateTime(localNow) < DayReviewEarliestLocalTime)
            return false;
        ...
        var memberContext = await _memberContext.ComposeAsync(
            new MemberContextRequest(member, memberId, reviewedDate, utcNow, PromptPurpose.DayReview), ct);

        var prompt = $"""
            {DayReviewPrompt.Instructions}

            {memberContext}
            {DayReviewPrompt.ReadingsSection(log, baseline, member.DateOfBirth.ToAgeInYears(reviewedDate))}
            """;

        var aiResponse = await _medicalAi.GenerateStructuredAsync<DayReviewAiResponse>(prompt, ct);
        var text = aiResponse.Summary.Trim();

        // Nothing is written rather than something wrong — the same stance as the family summary,
        // and with more behind it here. A day review is written once, so a bad one is not replaced
        ...
        if (text.Length == 0 || DayReviewPrompt.ReadsLikeTheInstructions(text))
        {
            _logger.LogWarning(
                "Discarded the day review for CardiMember {CardiMemberId} on {LocalDate}: the model "
        ...
        if (DayReviewPrompt.NamesACondition(text) is { } condition)
        {
            _logger.LogWarning(
                "Discarded the day review for CardiMember {CardiMemberId} on {LocalDate}: it names a "
        ...
        if (DayReviewPrompt.UnglossedTerm(text) is { } term)
        {
            _logger.LogWarning(
                "Discarded the day review for CardiMember {CardiMemberId} on {LocalDate}: it uses "
        ...
        {
            _logger.LogWarning(
                "Discarded the day review for CardiMember {CardiMemberId} on {LocalDate}: it names the "
        ...
        await _unitOfWork.Digests.AddAsync(new DigestEntry
        {
            ...
            Audience = DigestAudience.DayReview,
        ...
        // No question is asked off a day review. Questions exist to explain readings while they
    ...
    /// <summary>
    /// MedGemma's reply shape for the day review. Four fields, where the family summary has seven:
    ...
    /// </remarks>
    internal sealed record DayReviewAiResponse
    {
        [Description(

...
public partial class DigestGenerationService : IDigestGenerationService
{
    ...
    /// registered it still writes digests, and the many tests over the digest and journal paths
    /// need not stand one up to exercise them.
    /// </summary>
    ...
    /// </summary>
    private JournalComposition FinishJournalCopy(
        Guid memberId,
        DateOnly periodEnd,
        DigestAudience audience,
        DateTime utcNow,
    ...
    {
        var (bookName, periodPhrase) = audience switch
        {
            DigestAudience.Daybook => ("daybook entry", $"on {periodEnd:yyyy-MM-dd}"),
            DigestAudience.Weekbook => ("weekbook", $"for the week ending {periodEnd:yyyy-MM-dd}"),
            DigestAudience.Monthbook => ("monthbook", $"for the month ending {periodEnd:yyyy-MM-dd}"),
        ...
        if (requireMinimumSentences
            && JournalRegisterGuards.SentenceCount(text) < JournalRegisterGuards.MinimumSentences)
        {
            _logger.LogWarning(
        ...
        {
            _logger.LogInformation(
                "Glossed {Terms} in the {BookName} for CardiMember {CardiMemberId} {PeriodPhrase}.",
                string.Join(", ", glossed), bookName, memberId, periodPhrase);
            text = glossedText;
        }

        if (JournalRegisterGuards.UnglossedTerm(text) is { } term)
        {
            _logger.LogWarning(
    ...
    private async Task<bool> GenerateForMemberAsync(Guid memberId, DateTime utcNow, CancellationToken ct)
    {
        ...
        // And the member's reading against their own baseline, on the same terms: its own floor
        // and brief-version check decide whether this spends a model call, and this pass reached
        // here because the member's readings had moved — which is exactly when that reading is
        // worth rewriting.
        if (_insights is not null)
        {
            try
            {
                await _insights.RegenerateBaselineInsightAsync(memberId, ct);
            }
            catch (Exception ex) when (ex is not OperationCanceledException)
            {
                _logger.LogError(ex,
                    "Baseline insight regeneration failed for CardiMember {CardiMemberId}; the digest was stored.",
                    memberId);
            }
        }

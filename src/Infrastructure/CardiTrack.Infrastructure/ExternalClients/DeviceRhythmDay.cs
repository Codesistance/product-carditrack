...
/// <summary>
/// One analysis window out of an irregular-rhythm notification, as the provider served it:
/// the window's bounds, its AFib verdict, and every beat measured inside it.
/// </summary>
/// <param name="Beats">
/// Each beat's offset from <paramref name="StartUtc"/> in milliseconds, paired with its interbeat
/// interval in milliseconds. Empty where the provider served the window without beat detail, which
/// the schema permits — <c>heartBeats</c> is optional.
/// </param>
public sealed record RhythmAnalysisWindow(
    DateTime StartUtc,
    DateTime EndUtc,
    DateTime NotificationStartUtc,
    bool Positive,
    IReadOnlyList<(int OffsetMs, int RrMs)> Beats);
...
public sealed record DeviceRhythmDay(
    ...
    IReadOnlyList<RhythmAnalysisWindow> AnalysisWindows)
{

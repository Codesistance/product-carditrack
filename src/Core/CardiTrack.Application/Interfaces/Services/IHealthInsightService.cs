...
public interface IHealthInsightService
{
    ...
    /// dated record behind the single current suggestion <see cref="GetAdviseAsync"/> serves.
    /// Read-only and append-only at the source, so this never calls a model either. Throws
    /// <see cref="KeyNotFoundException"/> when the user may not view that member's health data.
    /// </summary>
    /// <param name="from">
    /// Start of the window, or null for the whole of what is retained. Clamped to the retention
    /// horizon: a caller asking for five years is told, in the response's own <c>from</c>, that it
    /// got one.
    /// </param>
    /// <param name="to">End of the window, exclusive, or null for now.</param>
    Task<AdviseObservationLogResponse> GetAdviseObservationsAsync(

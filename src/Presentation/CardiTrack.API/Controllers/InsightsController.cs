...
public class InsightsController : BaseApiController
{
    ...
    private readonly IValidator<AskMemberQuestionRequest> _askValidator;
    ...
    public InsightsController(
        ...
        IValidator<AskMemberQuestionRequest> askValidator)
    ...
    /// <summary>
    /// Answers a caregiver's free-text question about this CardiMember from the readings already
    /// on file. MedGemma only; the question is never persisted, never taken as an instruction,
    /// and cannot set an alert. 404 when the caller may not view the member — same
    /// non-disclosure as the other insight routes.
    /// </summary>
    [HttpPost("members/{cardiMemberId:guid}/ask")]
    [AuditHealthDataAccess("AskAboutMember")]
    [ProducesResponseType(typeof(ApiResponse<MemberAskResponse>), StatusCodes.Status200OK)]
    [ProducesResponseType(typeof(ErrorResponse), StatusCodes.Status400BadRequest)]
    [ProducesResponseType(typeof(ErrorResponse), StatusCodes.Status403Forbidden)]
    [ProducesResponseType(typeof(ErrorResponse), StatusCodes.Status404NotFound)]
    public async Task<ActionResult<ApiResponse<MemberAskResponse>>> AskAboutMember(
        Guid cardiMemberId, [FromBody] AskMemberQuestionRequest request, CancellationToken ct)
    {
        if (!UserContext.IsAuthenticated || UserContext.UserId == Guid.Empty)
        {
            return Error("We couldn't find your account — please sign in again.", StatusCodes.Status403Forbidden);
        ...
        var validation = await _askValidator.ValidateAsync(request, ct);
        if (!validation.IsValid)
            return ValidationFailed(validation);

        try
        {
            var result = await _insightService.AskAboutMemberAsync(

...
/// the member and audited. Questions are generated by the AI pipeline — there is no endpoint that
/// asks the model for one. Families can volunteer a standing fact without waiting to be asked.
/// </remarks>
[Authorize]
...
public class QuestionnairesController : BaseApiController
{
    ...
    private readonly IValidator<OfferStandingFactRequest> _offerValidator;
    ...
    public QuestionnairesController(
        ...
        IValidator<OfferStandingFactRequest> offerValidator)
    ...
    /// Not an ask — it does not occupy the pending slot and is never pushed.
    /// </summary>
    [HttpPost("cardimembers/{cardiMemberId:guid}/questionnaires")]
    [AuditHealthDataAccess("OfferStandingFact")]
    [ProducesResponseType(typeof(ApiResponse<QuestionnaireResponse>), StatusCodes.Status201Created)]
    [ProducesResponseType(typeof(ErrorResponse), StatusCodes.Status400BadRequest)]
    [ProducesResponseType(typeof(ErrorResponse), StatusCodes.Status403Forbidden)]
    [ProducesResponseType(typeof(ErrorResponse), StatusCodes.Status404NotFound)]
    public async Task<ActionResult<ApiResponse<QuestionnaireResponse>>> OfferStandingFact(
        Guid cardiMemberId, [FromBody] OfferStandingFactRequest request, CancellationToken ct)
    {
        if (!UserContext.IsAuthenticated || UserContext.UserId == Guid.Empty)
        {
            return Error("We couldn't find your account — please sign in again.", StatusCodes.Status403Forbidden);
        ...
        var validation = await _offerValidator.ValidateAsync(request, ct);
        if (!validation.IsValid)
            return ValidationFailed(validation);

        try
        {
            var result = await _questionnaires.OfferStandingFactAsync(
                UserContext.UserId, cardiMemberId, request.FactText, ct);

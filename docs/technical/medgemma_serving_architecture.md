...
| MS-10 | **Resolved 2026-09-08 (implemented): a structured reply that fills the ceiling is a runaway, not a budget problem, and the pass retried it every few minutes.** Datadog logs from `pipeline-jobs` (dev, 5–8 September) showed 79 `generate_structured` calls stopping with `done_reason: length` at 2048/2048 output tokens, 67 of them surfacing as "Summary generation failed" for one member (60) — roughly every five to twenty minutes, across eight deploys, each costing ~36 s of the shared GPU to write nothing. The prompt was 1869 tokens in an 8192 window, so there was room; every *finished* structured reply in 14 days was ≤1051 tokens (p99 459). A reply that runs to a ceiling several times its normal size is the 4B model looping inside the grammar-constrained decode, and MS-9's remedy text ("raise MaxOutputTokens or ContextTokens") was the wrong instruction for it. **Fix:** `MedGemmaClient` now raises a typed `AiReplyTruncatedException` (still an `HttpRequestException`, so every existing catch holds) carrying the token counts, and its log line says to compare the count with the operation's norm before raising a ceiling. `DigestGenerationService` catches it and writes a `MemberAiHold` row (`MemberAiHolds`, one per member per purpose): the family-digest read for that member is skipped for 2 h, doubling per consecutive failure to a 12 h cap, cleared by the first reply that finishes. No waiver cuts through the hold — the read that would describe an alert is the read that cannot finish. The ceiling stays at 2048: lowering it would cut real Weekbook/Monthbook replies. **Not yet done:** ~~read the held member's completion~~ — done, see MS-11. | Resolved |
| MS-11 | **Resolved 2026-09-09 (implemented): the loop is the model restating one block of sentences verbatim, at Ollama's default repetition penalty, which never looked far enough back to see it.** The ClinicalInspection completions *are* in Datadog (dev, `service:pipeline-jobs "clinical inspection"`): eight `generate_structured` completions of 7,700–11,200 characters between 6 and 9 September, every one the `finding` (or a Weekbook `summary`) starting as a per-reading recap and then repeating a sentence block word for word until `num_predict` ended it — e.g. a seven-sentence block ("The heart rate is slightly higher than usual, at 85 bpm. The heart rate variability is slightly lower than usual, at 33.6 ms. …") about ten times over, or "The week was mostly steady, with Wednesday being slightly apart due to a high heart rate variability reading." until cut. Two members, two prompts, two schemas, one failure shape — so it is the model and the sampler, not the members' data. The client sent only `num_ctx` and `num_predict`, leaving the sampler at Ollama's defaults: `repeat_penalty` 1.1 over the last 64 tokens. The repeated digest block is ~170 tokens, so the window had scrolled past the block's start before it came round again, and the Weekbook loop (~25 tokens, inside the window) shows 1.1 was also too weak. The prompt's own "do not recap every listed figure" is ignored even on reads that finish: both finished completions read that morning walk every reading in the activity row in order, and a walk down a list has no end once the model loses its place. **Fix:** `MedGemmaClient` now sends `repeat_penalty` and `repeat_last_n` on every request, from `AI:Private:RepeatPenalty` (1.15, `medgemma_repeat_penalty`) and `AI:Private:RepeatLastN` (512, `medgemma_repeat_last_n`) — and the same pair on the Rewrite slot's Ollama kind — with startup refusing a penalty below 1.0 or a window below -1. The `finding` field's description (schema and prompt alike) now states a bound, "at most 150 words, and never the same reading twice", so the recap has somewhere to stop; it is a description rather than a schema `maxLength` on purpose — a grammar-enforced cut would close the JSON around a looped read and hand the rewrite slot repetitive junk as if it were a finished read, which is exactly what MS-10's hold exists to keep from a family. The ceiling stays at 2048. **Not yet done:** confirm in Datadog after a few days of dev traffic that `done_reason: length` on `generate_structured` stops, and that finished-read length does not fall (the penalty is modest for that reason; 1.15 is a first measurement, not a derivation). | Engineering (verify) |
...
The bytes are checked three times on the way to the image: `scripts/fetch-medgemma-weights.sh` after downloading from upstream (`vendor-medgemma-weights.yml` runs it, then uploads to `carditrack-common-model-weights` — its own bucket, versioned and never lifecycle-deleted, because the builds bucket deletes at ten days), `deploy-medgemma-common.yml` after downloading from that bucket, and the Dockerfile once more before `ollama create` sees them. The build then asserts `ollama list` shows the exact name the hosts will ask for. No stage talks to a registry. Locally, the same script fills `weights/` and `docker compose` registers the model the same way.

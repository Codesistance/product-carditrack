...
flowchart LR
  ...
  W -->|"vendor sync"| GH
  WK -->|"HTTPS · Bearer · up to 25 calls per day-in-window (up to 20 daily + 5 granular) + 1 battery per pull · every 10 min"| GH
...
| **External** `health.googleapis.com` | Google Health API v4, Google OAuth 2.0 | HTTPS, Bearer tokens | up to **25** calls per day-in-window per connection on the Worker cadence (up to 20 daily + 5 granular), **plus one `pairedDevices` battery read per pull** — flat, not per day, and skipped entirely without the `settings` scope |
...
3. **Per day, 17 HTTP calls, up to 20**, in five groups (activity ∥ heart rate ∥ sleep ∥ additional, then exertion once sleep returns):
...
|---|---|---|
| Per project, daily | 86.4M requests/day (~1,000 QPS sustained) | Up to ~3,700 requests/connection/day at a 10-minute cadence (up to 25 a pull, 100 on the day's first) — ~37M/day at the 10,000-wearer design target, **43%**; up to ~4,300 (**49%**) where the settings and rhythm scopes are granted |
...
| **Per user, minutely** | **300 requests/min** (5 QPS standard; **2.5 QPS and 100 users max while unverified**) | Up to 25 on a routine pull, 100 on the day's first pull (4 and 10 more with the settings and rhythm scopes), plus up to 140 while a backfill chunk runs — inside it on volume |

Two things to keep in view, neither of which is about how often we poll:

- **The per-user QPS burst is over.** A day's snapshot fires up to 20 requests in one burst for one wearer, up to 16 of them in flight at once, against a 5/s standard reading and 2.5/s unverified. Volume is fine; shape is not. Capping the per-wearer fan-out is the fix, not a slower cadence. This is unrelated to and not fixed by the pagination pacing below — it is the wider daily-snapshot burst, still open.

...
- **Nothing enforces any of this.** `MaxRequestsPerSecond`, `DormancyThresholdPulls` and `DormancyBackoffFactor` are validated at startup and **read by nothing** — there is no governor and no dormancy backoff. `FitbitApiClient` has no `429` handling either: a throttle surfaces as `FitbitApiException`, marks the connection `SyncError` and aborts that pull.

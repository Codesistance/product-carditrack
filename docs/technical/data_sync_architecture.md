...
flowchart LR
  ...
  W -->|"vendor sync"| GH
  WK -->|"HTTPS · Bearer · 22 calls per day-in-window (16 daily + 2 rhythm + 4 granular) + 1 battery per pull · every 10 min"| GH
...
| **External** `health.googleapis.com` | Google Health API v4, Google OAuth 2.0 | HTTPS, Bearer tokens | **22** calls per day-in-window per connection on the Worker cadence (16 daily + 2 rhythm + 4 granular), **plus one `pairedDevices` battery read per pull** — flat, not per day, and skipped entirely without the `settings` scope. The rhythm pair is skipped without the `ecg`/`irn` scopes, and never runs on backfill days |
...
3. **Per day, 16 HTTP calls**, issued concurrently in five groups (activity ∥ heart rate ∥ sleep ∥ additional ∥ body):
   - **8 roll-ups** — `POST /v4/users/me/dataTypes/{steps|distance|active-minutes|total-calories|floors|sedentary-period|heart-rate|weight}/dataPoints:dailyRollUp`
   ...
   - **2 body** — `daily-heart-rate-variability` as a Daily record, `blood-glucose` as a sample series (the series, not the rollup: only it carries the day's *lowest* reading, which is the one the hypoglycaemia alert fires on)

   Peak in-flight is therefore ~15 requests for a single wearer. The Google Health per-user ceiling is 300 requests/minute, which this is comfortably inside on volume, but its QPS reading (5/s standard, 2.5/s for an unverified app) is **not** — see the quota note below.
3b. **Per routine-window day, 2 more calls for rhythm** — `electrocardiogram` and `irregular-rhythm-notification`, both `list`, issued **sequentially after** the snapshot and **skipped entirely** for a connection without the `ecg`/`irn` scopes (which is every connection authorised before 2026-08-22, until the wearer reconnects). Best-effort like the battery read: a failure is logged and the day stores without them rather than failing the sync, because the routine window re-reads today every ten minutes. **Backfill days get no rhythm read at all** — ECG's filter grammar supports only `interval.start_time >=` with no upper bound and no civil-time field, so an old day is reachable only by paging back through every later reading. ECG is fetched with a `fields` partial-response selector that excludes `waveformSamples`: CardiTrack stores counts and the device's classification, never the trace.

...
- **Mobile RUM:** ✅ **Re-enabled behind consent.** RUM was removed in PR #185 because events could not reach UK1; it now ships through `IntakeHost` (per-feature custom endpoints — see the [APM runbook](apm_setup_runbook.md) §5). What it collects is minimised at source: views are named by Shell route or page class (never `Page.Title`), automatic action tracking is off (control text includes member names), and automatic resource tracking is off (request URLs carry journal search text and caregiver invite tokens, and the SDK cannot rewrite them). **Residual:** `SessionSampleRate` is 100 — the reduction proposed here was made when collection had no consent gate; sessions now exist only for caregivers who opted in. Mobile **logs, traces and RUM** are opt-in — the SDK initialises at `TrackingConsent.NotGranted` and only Settings → Privacy → **Send diagnostics** raises it, applying at runtime via `DdSdk.SetTrackingConsent` and cleared on sign-out (`DiagnosticsConsent`, `MobileApm.cs`). `NotGranted` rather than the `Pending` this document previously proposed: Pending buffers events on the device awaiting a yes, which is collection without consent — the trade is that diagnostics from before the opt-in are unrecoverable.
...
| **Datadog / Better Stack** (APM, logs, traces) | Request paths w/ member GUIDs, exceptions, DB spans. **Mobile RUM sessions (opt-in; no action or resource tracking) and no session replays** — Session Replay was never enabled | Pseudonymous identifiers linked to a health service ⇒ treat as PHI-adjacent; Datadog offers BAAs; **Better Stack: verify or drop for prod** | DPA + SCCs; retention config; mobile logs, traces and RUM ship only after the caregiver opts in (§7.4) | 🔴 Scrub + consent-gate first; paperwork per D6 |

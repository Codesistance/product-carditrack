...
| 9 | Encryption service is sound (AES-256-GCM) but has **a single static key** — no rotation path, and crypto-shredding-based erasure is not currently expressible. The key-id half of this landed 2026-09-09: ciphertext is now `v1:{keyId}:{base64}`, so a stored value says which key wrote it (pre-envelope blobs are bare base64 and decrypt on the legacy path). That is the prerequisite for rotation, not rotation itself — there is still one key, and replacing it still strands everything encrypted under the old one. It now has **four** consumers — device OAuth tokens, `MedicalNotes`, questionnaire Q&A text, and `PushDeviceToken.Token` (looked up via a SHA-256 `TokenFingerprint`) — so the single-key limitation spans four field families; the envelope that migration needed now exists, and what remains is a second key and a re-encrypt path | `src/Infrastructure/CardiTrack.Infrastructure/Security/AesEncryptionService.cs:21-31` (envelope + key id), `:74-116` (encrypt/decrypt/parse); consumers: `Services/DeviceConnectionService.cs`, `ExternalClients/OAuthTokenRefreshService.cs`, `CardiMemberService`, `QuestionnaireService`, push-token persistence |

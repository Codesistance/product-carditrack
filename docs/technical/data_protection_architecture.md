...
| 5 | **[PARTLY CLOSED 2026-09-14 — live in dev, not prod]** **There is now an erasure path; there is still no general retention job.** DELETE endpoints for CardiMembers and device connections remain **soft deletes** — they flip `IsActive` and discard OAuth tokens — but erasure no longer depends on them: `IMemberErasureService` hard-deletes the 29 member-scoped tables in one transaction, `IAccountErasureService` closes an account (erasing the members it was the last active watcher of, releasing the ones somebody else still watches), and `RetentionWorker` runs both on a schedule once a caregiver's thirty-day window elapses, alongside the 90-day member-chat sweep. Partition-based retention for the sub-daily/pipeline tables is still `PartitionMaintenanceWorker`'s (granular 90 d, hourly rollups 13 mo, **digests and journal entries 7 mo**, assessments 90 d, environmental 90 d). **What is left:** the prod deploy (dev went out 2026-09-14, CI run 34901698217, with the first sweep in rehearsal) and the age-based rule §5.2 designs for `ActivityLogs`/`Alerts`/`PatternBaselines`, which today are removed only on erasure, never by age | `src/Worker/CardiTrack.Worker/Workers/` (nineteen workers, incl. `RetentionWorker.cs`, `PartitionMaintenanceWorker.cs`); `MemberErasureService.cs`, `AccountErasureService.cs`, `ChatRetentionService.cs`; `CardiMemberService.RemoveAsync`, `DeviceConnectionService.DisconnectAsync` |
...
| 9 | Encryption service is sound (AES-256-GCM) but has **a single static key** — no rotation path, and crypto-shredding-based erasure is not currently expressible. The key-id half of this landed 2026-09-09: ciphertext is now `v1:{keyId}:{base64}`, so a stored value says which key wrote it (pre-envelope blobs are bare base64 and decrypt on the legacy path). That is the prerequisite for rotation, not rotation itself — there is still one key, and replacing it still strands everything encrypted under the old one. It now has **nine** consumers — device OAuth tokens, `MedicalNotes`, questionnaire Q&A text (`MemberQuestionnaire.QuestionText` and `.AnswerText`), `PushDeviceToken.Token`, `MemberChatTurn.Content`, `MemberChatTurn.Charts`, `MemberChatTurn.PendingChange` (the alert-settings proposal awaiting a yes, 2026-09-17), `MemberChatSession.Theme` (the push token is looked up via a SHA-256 `TokenFingerprint`), and `AlertResponse.Note` (a caregiver's own words about what they did, 2026-09-22); the four member-chat fields arrived after this row last counted. Re-derive the list from the `IEncryptionService.Encrypt` call sites rather than trusting the number — that is how these three were missed. So the single-key limitation spans nine field families; the envelope that migration needed now exists, and what remains is a second key and a re-encrypt path | `src/Infrastructure/CardiTrack.Infrastructure/Security/AesEncryptionService.cs:21-31` (envelope + key id), `:74-116` (encrypt/decrypt/parse); consumers: `Services/DeviceConnectionService.cs`, `ExternalClients/OAuthTokenRefreshService.cs`, `CardiMemberService`, `QuestionnaireService`, `DigestGenerationService` (question text), `MemberChatService` (turn content, charts and the pending alert-settings proposal), `ChatThemeService` (session theme), `DeviceTokenService` (push tokens), `AlertService` (alert-response notes) |
| 10 | Telemetry ships request paths containing `{cardiMemberId}` GUIDs, exception payloads, and Npgsql spans to Datadog/Better Stack with no scrubbing or retention config. *(The mobile-RUM half of this finding — `TrackingConsent.Granted` at 100% session sampling — was first resolved by removal in PR #185, when this org's Datadog site was unreachable from the mobile SDK. RUM came back in PR #1253, routed to UK1 through `IntakeHost`, and since 2026-09-24 it is **off before sign-in, then on by default with an opt-out** (§7.4); views are named by route or page class, and action and resource tracking stay off. The consent question that survived on mobile logs/traces was closed by opt-in on 2026-09-14; since 2026-09-24 mobile telemetry is instead on by default with an always-available opt-out, disclosed in the Terms of Service and Privacy Policy, its lawful basis pending legal confirmation (DPIA R-A8) — see §7.4.)* | `src/Infrastructure/CardiTrack.Observability/ApmExtensions.cs:87-151`, `src/Presentation/CardiTrack.Mobile/Services/MobileApm.cs` |
...
| Medical information ledger (free text per line) | `MedicalEntries.Text` (AES-256-GCM, static key, `MedicalLedger`) | `pii.subject_identities`, encrypted with the subject DEK — the same content as the medical notes, one line at a time; `CardiMembers.MedicalNotes` is rewritten as their summary. Changed and removed lines are kept as the member's history; erasing a line (`DELETE .../medical-entries/{id}`) hard-deletes it (GDPR Art. 17). `Kind` is stored in the clear: it says only which heading a line sits under |
...
| Device connections + OAuth tokens | Postgres clinical | Life of connection; **tokens purged on disconnect/consent-withdrawal**, with provider-side revoke (`https://oauth2.googleapis.com/revoke`) — **both enforced in code since 2026-09-14** (#148 item 4): `IOAuthGrantRevoker` ends the grant at the provider before the token columns are cleared, on disconnect, on member removal and in the erasure cascade. A revocation the provider will not confirm is reported, not swallowed — after the row goes there is no token left to retry with | Hard delete of token columns; connection row hard-deleted at member erasure | Live credentials to third-party PHI |
...
| Stored insights (`MemberInsights`) | Postgres clinical | **90 days for the member-scoped rows** (baseline reading, and the rolling, weekly and monthly trend narratives — one row per member per scope), matching `RealtimeAssessments` rather than the journal's 7 months — an insight is a re-derivable interpretation, not a retained account of the member's condition. **Enforced by `RetentionWorker`** (`InsightRetention`), since the table is ordinary EF-tracked rather than partitioned and the drop mechanism does not reach it. **Alert-scoped rows are not swept** — they explain a fixed past event, are served however old, and leave with the member on erasure | Hard delete by `CardiMemberId` | Model-written text at five scopes: one explanation per alert, one reading against the member's own baseline, and three trend narratives at different horizons. The three trend scopes are one category at three cadences, not three categories — same path, guards, processor and source data. DPIA §6.3 carries the minimisation argument |
| Generation claims (`GenerationLeases`) | Postgres clinical | **No retention period, and none needed:** one row per member per `GenerationWork`, overwritten by the next period's claim rather than accumulating. The table is bounded at members × works by construction, which is why it is keyed without the period it holds | Hard delete by `CardiMemberId` | **No health data.** A member id, which writer holds the claim, which period it covers and two timestamps — what stops two overlapping pipeline executions both paying to generate the same period. Leaves with the member on erasure |
...
| Member chat (`MemberChatSessions`, `MemberChatTurns`, `MemberChatTurnUsages`) | Postgres clinical | **90 days** from a session's **last written turn** — decided 2026-09-14 ([#488](https://github.com/Codesistance/product-carditrack/issues/488)) and **enforced by `RetentionWorker`** (`IChatRetentionService`) — **in dev since 2026-09-14, first sweep in rehearsal; not in prod**. Not partitioned, so a worker sweep is the route rather than a partition drop; the cut-off is derived from the newest `MemberChatTurn` rather than `LastTurnAtUtc`, which a mere reopen bumps, and a session that never got a turn is dated from `StartedAtUtc` | **Hard delete the whole session**; turns and usages cascade | The first persisted, non-de-identified AI conversation here (DPIA A20). A deliberate minimisation choice, not an inference from the readings: a transcript is a live Q&A, not a retained account of the member's condition the way the CardiJournal books are, so it does not earn their seven months. It is **not** the case that the underlying data has gone by then — hourly rollups keep 13 months, raw daily activity 25, and chat also reads baselines and unresolved alerts; the point is that free-text questions and model answers about a named person are worth keeping for less time than the readings behind them, not that they stop being answerable. Whole conversations, because a thread that opens mid-answer is worse than no thread |
...
> **Partly built 2026-09-14 — as `RetentionWorker`, not under this name.** The first slice ships
> the two rules with a published deadline behind them: account erasure thirty days after the
> request (#148) and member chat ninety days after its last turn (#488). It takes the advisory
> lock this design requires, carries a `DryRun` switch and a bounded batch, and has a per-account
> error boundary — but it is **rule-shaped, not policy-driven**: there is no `RetentionOptions`
> policy array, no `compliance.retention_runs` evidence table, and no anonymize-in-place or
> archive-then-delete action. The design below is still the target, and the categories it covers
> that the worker does not — `ActivityLogs`, `Alerts`, `PatternBaselines`, notifications, push
> tokens — still have no age-based rule at all. Two notes for whoever finishes it: the erasure
> cascades are already factored behind `IAccountErasureService`/`IMemberErasureService`, so a
> policy engine should call them rather than restate their table order; and the advisory-lock key
> space is worth auditing first — `OrphanedPhotoCleanupWorker` and `QuietReassuranceWorker`
> currently share key `8_472_100_002`, which is a latent collision rather than a live one only
> because their crons are five hours apart.
...
"Workers": {
  "DataRetentionWorker": {
    ...
    "Policies": [
      ...
      { "Name": "member-chat",       "Table": "member_chat_sessions", "MaxAgeDays": 90,   "Action": "HardDelete",       "Enabled": true }
    ]
  }
}
```
...
| At rest (field) | AES-256-GCM, single static key; **nine** consumers (device OAuth tokens, `MedicalNotes`, questionnaire Q&A text (`MemberQuestionnaire.QuestionText` and `.AnswerText`), `PushDeviceToken.Token`, `MemberChatTurn.Content`, `MemberChatTurn.Charts`, `MemberChatTurn.PendingChange`, `MemberChatSession.Theme`, and `AlertResponse.Note`). **Envelope shipped 2026-09-09** as `v1:{keyId}:{base64}` (`AesEncryptionService`) — not the `keyId‖nonce‖tag‖ct` layout this column once planned, and labelled `v1` in code; pre-envelope blobs are bare base64 and decrypt on the legacy path | **Remaining:** a second key and rotation. The envelope records the id and *validates* it — `Decrypt` rejects a value written under a different key with a message naming both, rather than a bare tag-mismatch — but it never selects a key; routing on `keyId` arrives with the second key. Rotation = new key version + lazy re-encrypt on write. Vault payloads use per-subject DEKs wrapped by **Cloud KMS** (net-new Terraform: key ring + KEK + IAM binding to the identity service account only) |
| Immediate fix | ✅ **Done** — `MedicalNotes` is encrypted (closes §1.2), applied in `CardiMemberService` rather than as an EF value converter so pre-existing plaintext rows stay readable | The second reader has arrived — the prompt-context framework reads the column (§1.2 row 2) — so the value-converter move is due on its own terms rather than on that trigger. New values already carry the `v1:{keyId}:{base64}` envelope; what is left for this column is re-encrypting the rows written before it |
...
- **Mobile RUM:** ✅ **Re-enabled, on by default with an opt-out** (opt-in 2026-09-14 → 2026-09-24). RUM was removed in PR #185 because events could not reach UK1; it now ships through `IntakeHost` (per-feature custom endpoints — see the [APM runbook](apm_setup_runbook.md) §5). What it collects is minimised at source: views are named by Shell route or page class (never `Page.Title`), automatic action tracking is off (control text includes member names), and automatic resource tracking is off (request URLs carry journal search text and caregiver invite tokens, and the SDK cannot rewrite them). **Residual:** `SessionSampleRate` is 100 — the reduction proposed here was made when collection had no gate at all; sessions now stop for any caregiver who turns them off. Mobile **logs, traces and RUM** are on by default — the SDK initialises at `TrackingConsent.NotGranted`, nothing is sent before sign-in, and at sign-in it is raised to `Granted` unless the caregiver has turned off Settings → Privacy → **Send session telemetry**, which drops it to `NotGranted` at runtime via `DdSdk.SetTrackingConsent`; sign-out removes the stored choice and stops collection until the next sign-in, so one caregiver's "off" is not inherited by the next (`DiagnosticsConsent`, `MobileApm.cs`). Disclosed in the Terms of Service and Privacy Policy; and told once in the app by a one-time in-app notice on the dashboard (`TelemetryNotice`, 2026-09-24) that says what is sent and where to turn it off — acknowledged per caregiver on the phone, not a consent step. The off state is `NotGranted` rather than the `Pending` this document previously proposed: Pending buffers events on the device, so "off" would still record. Lawful basis (PECR reg. 6 as amended by the DUAA 2025 + UK GDPR 6(1)(f)) is pending legal confirmation, including EEA ePrivacy consent — DPIA R-A8.
...
| **Datadog / Better Stack** (APM, logs, traces) | Request paths w/ member GUIDs, exceptions, DB spans. **Mobile RUM sessions (off before sign-in, then on by default with an opt-out; no action or resource tracking; no name or email in the payload, but traces could join API traces tagged with the pseudonymous account ID if trace headers were sent — latent while resource tracking is off) and no session replays** — Session Replay was never enabled | Pseudonymous identifiers linked to a health service ⇒ treat as PHI-adjacent; Datadog offers BAAs; **Better Stack: verify or drop for prod** | DPA + SCCs; retention config; mobile logs, traces and RUM ship by default and stop when the caregiver turns them off (§7.4) | 🔴 Scrub + consent-gate first; paperwork per D6 |

...
One structured call on `AI:Rewrite` (Vertex), and it does **one job: classify**. It does not choose datasets or windows. The rendered purpose lines — eight now that `investigation` has its handler and `settings` has shipped, see §4 — plus a closed list of reading and advise-topic labels are the vocabulary it carries.

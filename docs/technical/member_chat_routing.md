...
6. **Shadow-route.** Log disagreement against the existing triage and how often close candidates appear. Ship nothing on its answer. *Built as `ChatRouting:Mode = Shadow`; by the 2026-08-24 decision it is a diagnostic to reach for, not a phase to pass through.*
7. **Cut over** `status`, `analysis`, `advise`, `steer.casual`, `steer.offtopic` and `clarify`. *Done — the cutover is the default (`On`), with `Off` as the one-config rollback.*

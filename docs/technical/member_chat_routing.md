...
- **Only an empty conversation is welcomed** (2026-09-26). `steer.casual` learns one content-free fact about the history — whether it has any turns. With none it greets and says what the chat can do; part-way through, it answers what was said in a sentence or two, with no greeting and no list, so a "thanks" after an answer is no longer met with "Hi there! I can help with…". Its partner fix: the chat reply no longer closes with an offer ("shall I look at…?"), because the bare "yes" it invites carries no topic for a router that sees questions only.

...
| `answer.updated` | after the turn is saved, only when the check's remedy changed the draft | the saved `MemberChatMessageResponse`; the app replaces the draft in place, labelled "Updated answer" |
| `done` | last | `{}` |

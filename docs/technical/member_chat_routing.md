...
- **A member with nothing to read is answered before the planner** (2026-09-25). The suggested *"Anything I should keep an eye on?"* about a member whose watch had never synced came back "Everything looks settled and steady… nothing to worry about", under a dashboard saying nothing had come through yet. A verdict question names no reading, so the coverage gate below had nothing to measure; the clinical read was handed no readings and a Green tier that only meant nothing had been graded. Now `analysis`, `inference` and `investigation` each start with one shared check (`MemberChatService.NoReadingsResultAsync`): a read of the last seven days of readings and the unresolved alerts. The week is counted on the member's own calendar as well as UTC's, from a week before the earlier of the two todays through the later one. Rows are dated on the member's anchor clock, so east of Greenwich a new member's first sync would otherwise sit past a UTC week's end for hours. With no reading in those seven days and no open alert, `MemberChatReplies.NoReadingsYetReply` answers in code, stamped with the rung that was routed. It has two shapes, because they are different news: a member who has never synced (the dashboard's `LastSyncedAt` — the member's stamp, else its active connections' — is null) "hasn't sent any readings through yet", and one who has synced before gets "no readings have reached us … in the last 7 days". Neither says why, for the reason the data-gap rule above gives. No planner, MedGemma or rewrite call is made; the turn bills the triage and the route. An open alert keeps the member answerable, since it is a finding the family is already looking at.
- **What the read is told when there is nothing.** Two prompt-side backstops for the member the check above lets through. A window that was read and came back empty is written as its own dated section, `Recent readings: none reached us from … to …`, saying that nothing in it can be called settled or steady. It used to be silent. And with nothing fetched at all, `FormatFetchedData` no longer says to "answer from the member context above": that context holds no readings. It says there is nothing to go on and that nothing may be called settled or steady.
- **The chip follows the data.** With no reading in a week and no open alert, the first suggestion chip is *"Has anything come through yet?"* rather than *"Anything I should keep an eye on?"*. (Since 2026-09-26 the standard set is three chips, and a caregiver with chat history about the member gets their own last three distinct questions instead; see `MemberChatService.GetSuggestionsAsync`.) It is a sync question, so it routes to `status`, whose code reply already says nothing recent has come through (`MemberChatReplies.LatestReadingsReply`).
...
- **Only an empty conversation is welcomed** (2026-09-26). `steer.casual` learns one content-free fact about the history — whether it has any turns. With none it greets and says what the chat can do; part-way through, it answers what was said in a sentence or two, with no greeting and no list, so a "thanks" after an answer is no longer met with "Hi there! I can help with…". Its partner fix: the rewrite no longer closes with an offer ("shall I look at…?"), because the bare "yes" it invited carried no topic for a router that sees questions only. Offers came back the same day written and held by code — see *Follow-up offers* under §7.
...
### Follow-up offers (added 2026-09-26)

An `analysis` or `inference` answer about exactly one reading ends with one offer: the neighbouring reading over the same days (sleep ↔ resting heart rate; steps, HRV and breathing rate → the one they are read beside). Code writes it from a closed table (`PendingChatOffer.For`), after the answer check so the check judges the answer alone, and holds it on the assistant turn as encrypted `MemberChatTurn.PendingOffer` — `{metric, days}`, nothing else.

- **What is offered is what chat can fetch.** Another reading over the same days, at most the whitelist's seven. "Further back" is never offered: chat reads a week by design.
- **No offer** on a general answer (no single reading), an answer the check found short of the question, the could-not-answer line, a reply with a change pending, or one with no room under the length cap.
- **A yes** (the shared `ConfirmationVocabulary`, so "yes please" and "ok" count; "yes but…" does not) to a current offer — the latest assistant turn's, inside `PendingChatOffer.Validity` — becomes the question code writes for it ("How has CardiTrackCardiMember slept this week?"), and that question runs the whole send: pre-check, router, workflow, check. The router still never reads a model's prose. The transcript keeps what the caregiver typed.
- **A no** to a current offer is answered in code, with no model call.
- **A bare yes or no with nothing to answer** — no offer, a lapsed one, or one a newer reply superseded — is answered in code too, asking what they would like. Routed, it reached `steer.casual` and was greeted as a new conversation.
- Anything else routes as itself, and the offer lapses. Offers are tagged `chat.offer` = `offered` / `accepted`; the code answers carry `route_source` `offer_declined` / `nothing_pending`.
...
| Event | When | Data |
|---|---|---|
| `step` | as each stage starts | `{ step, text, index, total }`: `understanding` (the pre-check passed), `planning`, `reading` (the clinical read), `rereading` (inference's second read), `writing`, `checking` (the answer check), `retrying` (the answer check's retry, followed by that attempt's own `planning`, `reading` and `writing`). `index` counts from 1; `total` is null on `understanding` and set from the next step on (see below) |
| `waiting` | at most once, on a path that reads the readings, if ready before the send settles | `{ sentences }`: up to four lines written for this question, the same shape `waiting-sentences` returns |
...
**Numbered steps (added 2026-09-25).** Each step carries where it falls (`index`) and how many the send is expected to take (`total`), so the app can show a progress bar. The count is taken from what is reported, never declared per workflow: a path that plans is five steps (understanding, planning, reading, writing, checking), a path that reaches the answer check without planning (status, advise) is done when it gets there, and `understanding` has no total because it is reported before the route that decides it. Two things add steps after the fact, and both grow the total rather than rewinding the count: inference's second look comes after the first write (one step), and the answer check's retry reports `retrying` and then that attempt's `planning`, `reading` and `writing` (four) (`MemberChatService.NumberedSteps`).

**Waiting lines on the long paths (added 2026-09-25).** The steps say what stage the send is at, but the clinical read alone sits on one step for half a minute or more. When a streamed send turns out to read the readings (its first `planning` step), the service starts the old waiting-sentences generation alongside the pipeline — the same Rewrite-slot prompt, the same redacted question, now asking for four lines about what this question asked — and sends them as one `waiting` event for the app to rotate under the step. It is cancelled the moment the send settles, so lines that lose the race are never sent, and a failure sends nothing rather than canned lines. Quick paths (steers, status, settings, journal) and the JSON endpoint never pay for the call. The generation takes the already-redacted message and touches only the Rewrite slot, never the unit of work, because it runs alongside a send using the same request's DbContext.
...
**Kept for older builds.** `POST …/messages` and `POST …/waiting-sentences` stay: an app built before streaming still calls both. The new app calls neither. Builds that stream but predate `waiting` skip it, as they skip any event name they do not know, and ignore `index` and `total`.
...
| Nothing to read is not the same as nothing wrong | "Everything looks settled and steady" about a member who had never synced (2026-09-25) | The no-readings check before the planner on every reading rung; an ungraded hero rendered Unknown; empty windows stated; the chip swapped |

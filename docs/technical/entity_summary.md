...
- **DigestAudience**: Family, Wearer, Daybook, Weekbook (Wearer generated only once wearer logins exist — currently never. Daybook is the once-daily account of a finished day: same family reader as Family, different reading-mode — see [llm_design.md](../llm_design.md)). `Weekbook` is the account of a finished week, dated by the week's last day, with its own partial unique index; `Monthbook` joins this enum when it is built (R2) — the audience persists as its name, so new values cost no migration

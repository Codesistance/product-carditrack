...
- Distinct from `AlertPreference`, which toggles CardiTrack's own eleven rules: that one is keyed by compile-time catalogue strings, this one by Guid, and `AlertRuleOverrides` drops ids its catalogue does not know

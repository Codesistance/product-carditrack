...
- **DigestAudience**: Family, Wearer, Daybook (Wearer generated only once wearer logins exist — currently never. Daybook is the once-daily account of a finished day: same family reader as Family, different reading-mode — see [llm_design.md](../llm_design.md)). The CardiJournal's other books, `Weekbook` and `Monthbook`, join this enum when they are built (R2) — the audience persists as its name, so new values cost no migration

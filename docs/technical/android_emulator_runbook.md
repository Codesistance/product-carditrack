...
| `Platforms/Android/google-services.json` *(only for push)* | git-ignored Firebase config, fetched from Secret Manager: `gcloud secrets versions access latest --secret=carditrack-common-firebase-android-config --project=carditrack-490120 --out-file=src/Presentation/CardiTrack.Mobile/Platforms/Android/google-services.json` (`--out-file`, not a `>` redirect, which PowerShell re-encodes). Without it the app builds and runs but never registers for push (see the [readme's Firebase config note](../apps/mobile/readme.md#push-notifications)). The key in the file is restricted by signing certificate, so your Debug keystore's SHA-1 must be on it too — [store provisioning §G](../apps/mobile/store_provisioning.md#g-firebase-client-config) |
...
3. **The reviewer may be driving the same emulator.** During a live review the owner navigates the device while sending instructions, so a blind coordinate script lands on whatever screen they are on and "verifies" their page, not yours. Screencap and check the page before every tap sequence, keep scripts to one screen per run, and re-check after each navigation step. A screenshot of a page you did not open means they are on it — do not tap.
4. **The dev API may be behind the client.** A branch that adds fields to a response the app reads will see them as defaults on the emulator until that API is deployed to dev (dispatch-only, after merge). To see the new client states anyway, fake the fields in the page's `Apply` behind a `// PREVIEW HACK — do not commit` comment (pin any fake timestamps, or a read/unread transition can never be reached), screenshot, then `git checkout` the file before the next real deploy and say in the PR which values were faked.
5. **Screenshot after every change** and check the affected screen before reporting it done. The review loop the owner expects is: make the change → rebuild + redeploy → screenshot → one-line confirmation. Commit per concern to the one open PR.
6. **Say what you could not verify.** OAuth round-trips, push notifications, and data states that do not exist on dev cannot be exercised on the emulator; state that plainly in the PR body and ask for confirmation on a real device.
7. **Never commit `Local.props`, credentials, or screenshots containing account emails.**
8. **Clean up.** When the task's PR is merged, remove the sibling worktree (`git worktree remove`), delete the local branch, and delete the remote branch. If `Remove-Item` fails part-way with `DirectoryNotFoundException` under `obj/Debug/net10.0-android`, that is `MAX_PATH` on delete: mirror an empty directory over it with `robocopy "$env:TEMP\empty" "<worktree>" /MIR` (PowerShell), then delete the husk and `git worktree prune`.

...
| `Platforms/Android/google-services.json` *(only for push)* | git-ignored Firebase config, fetched from Secret Manager: `gcloud secrets versions access latest --secret=carditrack-common-firebase-android-config --project=carditrack-490120 > src/Presentation/CardiTrack.Mobile/Platforms/Android/google-services.json`. Without it the app builds and runs but never registers for push (see the [readme's Firebase config note](../apps/mobile/readme.md#push-notifications)) |

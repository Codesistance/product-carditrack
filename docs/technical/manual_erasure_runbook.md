...
| 19a | `BenignJudgements` | The statistical pass's record that the model judged a finding not worth the family's attention — member id, rule id, local day and a hash of the finding's figures. **No health data on the row**: the hash is one-way and the figures it was taken over are not stored. Swept after seven days on its own, and has no foreign key to cascade behind it, so it must be deleted explicitly |

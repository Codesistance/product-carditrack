...
- The schema is deliberately almost free of foreign keys. Only `UserCardiMembers` and `Subscriptions` cascade. Deleting a CardiMember **orphans** its `ActivityLogs`, `Alerts`, `PatternBaselines`, `DeviceConnections` and `AuditLogs` rows, which stay live and queryable — and deleting an `Alert` orphans its `AlertResponses`, which hold encrypted free text about the member. The one place that absence actively bit — an AI generation writing health data back *after* the cascade had passed its table — now has a compensating control; see the next bullet.
- **Erasure is now safe against a generation already in flight** (2026-09-21). This matters because it used to be the opposite. Every AI writer reads the member, calls a model for seconds to minutes, then writes a row naming them; with no foreign key, a generation that started before the erasure could land its row after it, and the member would be erased with a fresh digest, insight, advise line, assessment, alert, chat turn or report export sitting behind them. `IMemberErasureService` now takes `SELECT … FOR UPDATE` on the `CardiMembers` row as the **first statement of its transaction**, and every writer holds `FOR KEY SHARE` on the same row for the length of its write (`IMemberWriteGuard`). One of two things therefore happens and both are safe: the generation finds no row to lock and writes nothing, or it wins and the erasure waits for it and then sweeps what it wrote. **For an operator this means the row counts in the report are still the whole story** — you do not need to re-check the generated tables after a sweep, and a `DigestEntries` or `MemberInsights` count that is higher than you expected is a generation the erasure correctly waited for, not a leak. Note this protects rows, not the two things that were never transactional: an unrevoked grant and an orphaned storage object still need the checks below — though one class of orphaned object is now closed too: the report object names are read **under the member lock, inside the transaction**, so an export uploaded by a generation that won the race is named in the report rather than left in the bucket with nothing pointing at it.
...
**Six columns reference a caregiver without owning the row:** `Alerts.AcknowledgedByUserId`, `Alerts.ResolvedByUserId`, `MemberQuestionnaires.AnsweredByUserId`, `AlertResponses.UserId`, `MedicalEntries.AddedByUserId` and `MedicalEntries.RemovedByUserId` (all nullable). When a caregiver's account is erased but the member stays — another caregiver remains linked — set those to null rather than deleting the member's alert or answer; the `LIKE '%UserId'` clause above is what surfaces them, so do not narrow it back to the exact names.
...
| 4a | `AlertResponses` | What caregivers said they did about this member's alerts, keyed on `AlertId` rather than `CardiMemberId` — so it is invisible to a `LIKE '%CardiMemberId'` sweep and must be joined through `Alerts`. Each row holds an **encrypted free-text note** about the member. Before `Alerts`, or it is orphaned |
...
| 14a | `MedicalEntries` | The medical-information ledger: current lines **and the history** of changed and removed ones, AES-256-GCM encrypted at rest. No foreign key, so nothing cascades it. `CardiMembers.MedicalNotes` is only its summary — deleting the member row does not remove these |
...
| 19a | `BenignJudgements` | The statistical pass's record that the model judged a finding not worth the family's attention — member id, rule id, local day and a hash of the finding's figures. **Derived health data**: the rule is stored in the clear, so a row naming `ecg_afib` or `irregular_rhythm` discloses a device rhythm finding for that member on that date, and the hash is a cache key rather than a privacy control (unsalted, over low-entropy figures). Swept after seven days on its own, and has **no foreign key to cascade behind it**, so it must be deleted explicitly |
...
| 40 | `Alerts.AcknowledgedByUserId`, `Alerts.ResolvedByUserId`, `MemberQuestionnaires.AnsweredByUserId`, `AlertResponses.UserId`, `MedicalEntries.AddedByUserId`, `MedicalEntries.RemovedByUserId` | **Null, do not delete** — the alert, the answer and the medical-information line belong to the member. `AlertResponses.UserId` was made nullable for exactly this: the note says what was done about somebody still being watched, and is what stops the next caregiver repeating a phone call this one already made |

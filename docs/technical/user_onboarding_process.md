...
- [x] In-app disclosure in the **mobile app** — shipped 2026-09-13: `HealthDataDisclosureBanner` on the Dashboard shows the same prescribed sentence, links to the privacy policy in the in-app reader, and hides only once `POST /api/v1/users/me/health-data-disclosure/dismiss` has recorded the acknowledgement on the account (same `HealthDataDisclosureDismissedDate` the web banner writes)
...
└── ~34 nullable metrics: Steps, HeartRate, Sleep stages, SpO2, VO2Max, HRV, overnight breathing, HR-zone minutes, longest still stretch, ...

PatternBaselines (Written daily by BaselineCalculationWorker)
Alerts (Generated by the pipeline's StatisticalAlertService + RealtimeAssessmentService, and the Worker's InactivityDetectionWorker)
...
// (moved 2026-09-19: StatisticalAlertService now runs in the pipeline's assess job, findings judged by MedGemma)

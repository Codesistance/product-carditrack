...
The image is built from `src/Infrastructure/MedGemma/Dockerfile`, which bakes the model tag from `.model-version` (`hf.co/unsloth/medgemma-1.5-4b-it-GGUF:Q4_K_M`) into it. That tag is the single source of truth for *which* model: `docker-compose.yml` pulls it locally, and `main.tf` reads the same file into `AI__Private__Model` rather than repeating the string, so the two cannot drift.

**Which build of it** is `.model-digest`, beside it. The tag is mutable and cannot be pinned at pull time — neither registry Ollama pulls from serves a manifest by digest, and `ollama pull` rejects a `model@sha256:…` reference outright (verified 2026-09-21) — so the pin is on the result instead: the build hashes the manifest Ollama stored and fails if it is not the recorded value. The manifest lists every layer by digest, so the one hash covers weights, prompt template and sampler params together. The running service was never the exposure here, because weights are baked into an image CI deploys by immutable tag; the rebuild was, and this closes it. The base image is pinned by digest for the same reason.

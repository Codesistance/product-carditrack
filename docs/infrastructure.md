...
| **AI pipeline (live in dev)** | Same Cloud SQL instance — **typed, day-partitioned result tables** for derived AI outputs | MedGemma assessments (`RealtimeAssessments`), family digests (`DigestEntries`), granular series and rollups — see [llm_design.md](./llm_design.md) |
...
| `DeviceActivityLogs` | Per-device raw daily rows, merged into `ActivityLogs` |
...
| `GranularMetricHours` | Minute-grain sub-daily samples as day-partitioned hour vectors |
| `MetricRollupsHourly` | Hourly rollups over the granular series (partitioned) |
| `DigestEntries` | AI family-digest generations, kept as history (partitioned) |
| `RealtimeAssessments` | MedGemma real-time heart-rate assessments (partitioned) |
| `MemberQuestionnaires` | Family questionnaire answers per CardiMember (encrypted Q/A text) |
| `MedicalEntries` | Medical-information ledger per CardiMember: one encrypted line per condition, allergy or medication, with author, confirmation date and a history of changed/removed lines; `CardiMembers.MedicalNotes` is kept as its summary |
| `EnvironmentalReadings` | Environmental (weather/air) readings per member locale (partitioned) |
| `Alerts` | Health alerts generated by pattern analysis |
| `AlertPreferences` | Per-CardiMember alert-rule enablement (sparse disable-list; missing row = all on) |
| `AuditLogs` | Compliance audit trail for PHI access |
| `Subscriptions` | Billing state per organization |
| `Notifications` | In-app notification inbox rows (data-completeness nudges, alerts) |
| `NotificationMutes` | Per-user mute/snooze state for notification rules |
| `NotificationRunLogs` | Bookkeeping for data-completeness reconciliation runs |
| `NotificationDeliveries` | Push delivery outbox (`FOR UPDATE SKIP LOCKED` claims, escalation state) |
| `PushDeviceTokens` | FCM device registration tokens with liveness tracking |
| `NotificationPreferences` | Per-user delivery preferences (quiet hours, categories) |

**Key recent migrations** (full history in `src/Infrastructure/CardiTrack.Infrastructure/Migrations/`):

- `AddAlertPreferences` (2026-08-15) — per-CardiMember alert-rule enablement (`DisabledRules` jsonb)
...
The image is built from `src/Infrastructure/MedGemma/Dockerfile` from inputs that all live in that directory: a `Modelfile` (template and params, in git), `weights.sha256` (the pin), and `weights/` (the GGUFs). The name it registers — `medgemma-1.5-4b-it:q4_k_m` — comes from `.model-version`, which `main.tf` reads into `AI__Private__Model` and `docker-compose.yml` uses too, so the three cannot drift.

**The weights are ours.** They used to be pulled from a Hugging Face tag at build time — a mutable third-party pointer, and one that stopped working on 2026-09-21 when Ollama's CVE-2026-85180 fix refused the cross-host blob redirect that tag's downloads arrive by. Now `vendor-medgemma-weights.yml` fetches the two GGUFs once from a pinned upstream commit (`weights.env`), checks them against `weights.sha256`, and keeps them in `carditrack-common-model-weights` (common stack: versioned, never lifecycle-deleted, unlike the builds bucket) under `<model>/<sha256>/<file>`. The deploy workflow downloads them from there, checks the hashes again, and the Dockerfile checks them a third time before `ollama create`. No stage reaches a registry. The hashes were confirmed from two independent sources — the upstream LFS objects and the layer digests of the tag previously served — so the vendored model is byte-identical to what has been running. The base image is pinned by digest for the same reason. A rename of the registered name is rolled out in three steps because callers get the name from Terraform and the image deploys separately: build with the old name in `.model-aliases` and deploy the image, apply the environment stack, then drop the alias.
...
| `ack-token-key` | Terraform (`random_bytes`, 32 bytes) | HMAC key for push ack/fetch tokens — API and Worker only (the pipeline never sends push directly) |
| `webhook-secret` | Terraform (`random_password`; only when `enable_webhook_receiver`) | `Webhook__Secret` — the full Authorization header value registered in the Google Health Subscriber's `endpointAuthorization.secret` |
...
├── datadog/                   # Datadog assets applied via its REST API: monitors/ (alert
│                              # monitors) + pipelines/ (the OTel severity-remapping log pipeline)
└── deployments/               # Single module with all per-env resources
    ├── main.tf
    ├── apis.tf                # Service enablement
    ├── alerting.tf            # Cloud Monitoring alerts (Cloud Run OOM, TLS cert expiry)
    ├── cloud_run.tf           # api, web, worker, webhook receiver + migrator/pipeline jobs
    ├── cloud_sql.tf
    ...
    ├── firebase.tf            # Firebase/FCM provisioning for the push spine
    ├── load_balancer.tf       # GCLB + CDN + Cloud Armor (domain-gated)
    ├── networking.tf          # VPC, subnet, private services access
    ├── pubsub.tf              # enable_pubsub (on in both environments)
    ├── redis.tf               # optional (enable_redis)
    ├── secret_manager.tf
    ├── service_accounts.tf    # Per-service runtime identities + their IAM grants
    └── outputs.tf
```
...
- **Log volume and trace sampling are per service.** The tfvars `log_minimum_level` (→ `Serilog__MinimumLevel__Default`) and `traces_sample_ratio` (→ `Apm__TracesSampleRatio`) are objects with one optional attribute per service (`api`, `web`, `worker`), so a single service can be turned up for an investigation. Prod baseline: **`Warning` and `1.0`**, matching the committed `appsettings.json` of all three services; **dev deliberately runs `api` and `worker` at `Information`** (web stays at `Warning`, traces at `1.0` everywhere). A partial override (`log_minimum_level = { api = "Debug" }`) leaves the other two at `Warning`; an unrecognised level or an out-of-range ratio fails the plan rather than being silently coerced.
...
| Service | Dev | Prod |
|---------|-----|------|
| API / Web | 0–1 instances, 1 vCPU / 512 Mi | 1–3 instances, 2 vCPU / 1 Gi |
| Worker | min **1** instance, 1 vCPU / **1 Gi** (raised after the 2026-08-11 OOM incident) | min **1** instance, 2 vCPU / 1 Gi |
| MedGemma | shared across environments: `carditrack-common-medgemma`, 0–**1** instance (Ollama single-instance), 4 vCPU / 16 Gi + 1× L4 | same shared instance |
| Webhook receiver | 0–N instances (scale-to-zero; no business logic, publish-only) | not deployed (`enable_pipeline_jobs = false`) |
| Pipeline jobs (digest / aggregator / assessor) | Cloud Run **jobs**, one execution per Cloud Scheduler tick — no autoscaling | not deployed |

Prod keeps a warm minimum instance for API and Web; dev scales those to zero. **The Worker's minimum is 1 in every environment** (`worker_min_instances`) — its crons live in an in-process timer loop, so a scaled-to-zero Worker runs no jobs at all.

### Background jobs

`CardiTrack.Worker` runs the 13 non-AI cron jobs as hosted services inside the Worker Cloud Run service: wearable sync polling (every 10 minutes), orphaned-organization cleanup (nightly), orphaned-photo cleanup, daily baseline recalculation, hourly partition maintenance/retention, weekly device-sync audit, inactivity detection, statistical alerting, device-auth recovery, questionnaire expiry, daily data-completeness reconciliation, the 30-second notification dispatch pump, and the push canary. The AI pipeline's scaling model is described in [llm_design.md](./llm_design.md).
...
- **Cloud Run** — request-based billing; dev scales to zero, prod keeps one warm instance per service **except MedGemma**, which is no longer a per-environment service at all: one shared GPU instance (`carditrack-common-medgemma`, common stack) serves every environment and scales to zero between calls (`medgemma_min_instances = 0`, `common/variables.tf`). The trade is a measured **~54s cold start** charged to the first caller after any idle period ([medgemma_serving_architecture.md](./technical/medgemma_serving_architecture.md) §9.1a) — acceptable for the batch passes, hidden from the interactive path by the app-open warm-up (§9.5). Warming does **not** make the prompt cheap, and nothing will: Gemma 3 uses sliding-window attention and `llama.cpp` will not restore a KV checkpoint under SWA, so every call reprocesses the whole prompt from token zero regardless of how long the instance has been up (`cached n_tokens = 0` on every generation, measured 2026-08-13 — see the prefix caching note in [llm_design.md](./llm_design.md)). Prompt length is the only lever on inference latency.

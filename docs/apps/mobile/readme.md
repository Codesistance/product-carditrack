...
Store versioning is stamped by CI at build time: `ApplicationDisplayVersion` comes from the computed semver tag and `ApplicationVersion` (iOS build number / Android versionCode) from the monotonic commit count — the values in the csproj are placeholders. The push workflow reports the same pair, read back from the tag. Pushing a tag twice therefore sends the same build number, and the store rejects the duplicate. The bucket keeps a tag's builds for 10 days; an older tag has to be rebuilt before it can be pushed.

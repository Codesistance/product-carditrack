...
- **Call / Send Message dim** when the CardiMember has no `Phone`. They stay tappable — a dimmed tile on touch has no hover state to explain itself with — and both the tap and `ToolTipProperties` say so. Note that **nothing in the app sets `CardiMember.Phone` yet** (the add/edit forms capture only `EmergencyContactPhone`), so today this is the state every member is in.
...
- **Engine selection**: `AppConfig.ApmEngine` names an entry in the registry — currently **Datadog only**. `ApmData` carries that engine's client-side connection JSON (`{"ClientToken":"pub...","ApplicationId":"...","Site":"Uk1","CustomEndpoint":"https://browser-intake-uk1-datadoghq.com"}`), stamped by CI as base64 (raw JSON accepted from `Local.props`). Client token and application id are write-only identifiers, safe to embed.
...
- **Datadog config**: `Datadog.Maui` package on **Android/iOS only** (no Windows support); site defaults to **Eu1** when unset; `SessionSampleRate` 100; `NativeCrashReportEnabled`; service name `carditrack-mobile`; environment tag derived from the API base URL (dev/prod).
- **Site vs `CustomEndpoint`**: `Datadog.Maui` 0.2.0's `DatadogSite` enum names only `Us1`/`Us3`/`Us5`/`Eu1`/`Ap1`/`Ap2`/`Us1Fed`. **Our org is on UK1, which the enum cannot name**, so `CustomEndpoint` (`https://browser-intake-uk1-datadoghq.com`) routes Logs and RUM to the right intake. A `Site` the enum does not recognise disables monitoring unless `CustomEndpoint` accompanies it — previously it fell back to `Eu1` in silence, which shipped telemetry to an unrelated org. Only 0.2.0 has ever been published, so a package bump won't remove the workaround.

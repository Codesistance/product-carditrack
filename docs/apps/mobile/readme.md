...
- **Channels** (Android): `carditrack.safety.v2` at IMPORTANCE_HIGH (wakes from Doze, bundled `carditrack_alert` sound + vibration), `carditrack.health.v2` at Default (same sound, shorter vibrate), `carditrack.nudges` at Low. v2 replaced the original ids because Android freezes a channel's sound on first create. iOS plays `carditrack_alert.wav` from the app bundle for Safety/Health.

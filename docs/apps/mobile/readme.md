...
Pushing a tag that has already shipped still sends a build number both stores will refuse. The push workflow now asks each store what it holds before it uploads anything and fails with the tag named and the remedy spelled out, rather than discovering it from a rejected upload per platform. A store that cannot be reached is not a veto — the upload step remains the backstop. The bucket keeps a tag's builds for 10 days; an older tag has to be rebuilt before it can be pushed.

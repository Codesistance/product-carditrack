...
- **One member's failure costs that member the tick, nothing more.** The pass runs on one scope; a failed save clears the change tracker before the next member so the failure is not replayed into every later save.

...
The only thing on this platform that deletes health data because time has passed rather than
because somebody asked. Two rules, both of them published commitments:

- **Accounts, 30 days.** The privacy policy and the deletion page promise erasure *within* 30 days
  of a verified request. The worker erases every account whose `DeletionRequestedAtUtc` is that old
...
- **A refused grant is "needs reconnecting", not silence.** While any of the member's non-suspended devices is `TokenExpired`/`AuthError` (`NeedsReconnect`), no device-silence alert is raised and none is probed; at the moment one would have been raised, the worker asks `INotificationGapResolver` to evaluate the member, which opens the Safety nudge `DEVICE_AUTH_BROKEN` (pushed by `NotificationDispatchWorker`'s sweep with a "Device needs reconnecting" teaser). A silence alert already standing is resolved when the refusal is found, at any hour; a probe whose own refresh is refused hands over the same way. Once reconnected, silence is reported as silence again. See `docs/technical/notification_engine.md` §3.

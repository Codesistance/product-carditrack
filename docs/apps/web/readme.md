...
1. **Serilog** — console sink always, `AddApmShipping` when the `Apm` engine is configured; `Npgsql.EnableLegacyTimestampBehavior` disabled (UTC everywhere).
2. **APM tracing** — `AddApmTracing("CardiTrack.Web")` (no-op until `Apm__Engine` + `Apm__Data` are set).

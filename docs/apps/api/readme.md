...
- PHI access is audit-logged via the opt-in `AuditHealthDataAccess` attribute, applied on **ten** controllers as of 2026-09-13: Alerts, CardiMembers, Dashboard, Devices, Insights, MemberChat, MetricAlarms, Onboarding (member creation only), Questionnaires, Reports (controller-wide on some, per-action on others — read the attribute, not this list). Repeated GETs from the same caller coalesce into one row per 15-minute window (`AuditLoggingMiddleware.ReadSessionWindow`). Unannotated endpoints (Auth, Notifications, the assistant readiness check and the A4 chat) are **not** audited; onboarding's CardiMember creation has been audited since 2026-09-13 (`CreateCardiMember`). **Retention policy is 6 years**; the deployed infrastructure currently retains 30 days (dev) / 90 days (prod) via tfvars — extending it to the policy horizon is tracked follow-up infra work
...
> The `GoogleHealth` provider block runs on the **Google Health API** (Google OAuth endpoints, `googlehealth.*` scope URIs, ~1-hour access tokens) and serves every device brand in its `DeviceTypes` list — Fitbit and Google Pixel Watch today. `RedirectUri` is the provider-facing **https bounce endpoint** — Google web OAuth clients cannot redirect to a custom scheme, so `GET /api/v1/oauth/redirect/fitbit` 302s back into the app deep link. `AdditionalAuthorizationParams` carries Google's `access_type=offline` (required for a refresh token); `FirstConsentAuthorizationParams` adds `prompt=consent select_account` to every add or replace, and to a reconnect of a connection with no refresh token or a failed grant (see [devices.md](../../execution/backend/api/devices.md)). Client id/secret come from Secret Manager (`devices-fitbit-client-id` / `devices-fitbit-client-secret` — historical slug, GoogleHealth block).
...
- **`Dockerfile`** — the API service itself, deployed to **Cloud Run** (binds the base image's `ASPNETCORE_HTTP_PORTS=8080`). Runtime is `mcr.microsoft.com/dotnet/aspnet:10.0-noble-chiseled-extra`, non-root UID 1654 — the `-extra` variant carries ICU and tzdata, which the plain chiseled base lacks. It carries **no fonts**, so a `fonts` stage installs `fonts-noto-core` on the SDK image and copies a ~4 MB subset of Noto Sans (one face per script: Arabic, Hebrew, Devanagari, Bengali, Gurmukhi, Tamil, Thai, Armenian, Georgian, Symbols) into `/app/fonts`; `ReportFonts.Register()` loads them at startup and the PDF export names them as the fallback chain after QuestPDF's bundled Lato, so a member's name prints in its own script rather than as boxes. The directory is optional — absent on a developer machine, startup registers nothing. Noto Sans CJK is deliberately left out (~70 MB resident); `ReportFonts.cs` says how to add it.

...
| Weight | `weight` — `dailyRollUp` (`weightGramsAvg`) | Per weighing, sparse | **Built 2026-08-22.** Not an SSA input — read as a multi-day delta by the `rapid_weight_gain` rule, which is a fluid-retention signal rather than a rhythm one |
| Blood glucose | `blood-glucose` — `list` | Per reading, sparse | **Built 2026-08-22.** Listed rather than rolled up: only the series carries the day's *lowest* reading, and the low is what the hypoglycaemia alert fires on |
| ECG | `electrocardiogram` — `list` (session-shaped) | Per reading the wearer takes | **Built 2026-08-22.** Classification only — the `waveformSamples` array is excluded by a `fields` selector and never stored. Its filter admits only `interval.start_time >=` (no upper bound, no civil field), so it is read for the routine window and never for backfill |
| Irregular rhythm | `irregular-rhythm-notification` — `list` (session-shaped) | Per notification the device raises | **Built 2026-08-22.** Counted per notification, not per `alertWindows` entry |

> **The rhythm pair needs its own scopes.** `googlehealth.ecg.readonly` and `googlehealth.irn.readonly` are separate from the three read bundles, and Google classes both data types as SaMD features. Every connection authorised before 2026-08-22 lacks them until the wearer reconnects, so both counts are null — never zero — for those members, and both alert rules stay silent rather than reassuring. Requesting them puts these scopes into the same restricted-scope verification track as the rest (issue #39).
>
> **HRV, weight and blood glucose need no new consent**: all three read under `health_metrics_and_measurements`, which every connection already holds. Weight and glucose depend on a connected scale or meter feeding the same provider account, so they are null for most wearers — a fact about their kit, not a failed read.

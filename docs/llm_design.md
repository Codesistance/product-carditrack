...
| `CARDITRACK_STATISTICAL_JUDGEMENT_PROMPT` | Every assessor run, for members with a finding that survived cooldown and dedup | Family members (as alerts) | Tone and `CaregiverRegister`, then the findings as JSON — rule, observation with figures and the yardstick crossed, the stored metric values — and, per finding, a strict severity token, a two-to-six-word headline and 1–3 caregiver-actionable sentences that name no day or clock time. **Built (2026-09-19)** in `StatisticalAlertService`; replaces the per-rule severity and copy constants the Worker used to write |

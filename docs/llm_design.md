...
| `digest_log` | `wearer_user_id`, `date`, `audience` | `digest_text` | **Built as the typed, day-partitioned `DigestEntries` table** (`CardiMemberId`, `LocalDate`, `Audience`, `GeneratedAtUtc` PK; family is the only audience today) — 90 days (3 months) by partition drop |

...
> - **Still design-only:** the digest's own push (a new summary does not page). *Trend interpretation is no longer design-only — it shipped in dev on 2026-09-20 and gained its weekly and monthly horizons on 2026-09-21; see the `TrendInterpreter` row below and the section at the end of this document.* Push delivery itself is **built** — the notification engine's FCM HTTP v1 relay with APNs passthrough, escalation ladder and quiet hours — and the assessor's red/orange path now POSTs to `/api/v1/internal/notifications/enqueue`. Medium (yellow) stays summary-only.
...
| Property | Value |
|----------|-------|
| Model | MedGemma 1.5 4B instruction-tuned, Q4_K_M GGUF (`medgemma-1.5-4b-it-Q4_K_M.gguf` + `mmproj-F16.gguf` from `unsloth/medgemma-1.5-4b-it-GGUF` @ `3855f948`), vendored into our own bucket and registered at image build as `medgemma-1.5-4b-it:q4_k_m` |
| Parameters | 4B |
...
| `TrendInterpreter` | Cloud Scheduler | **Daily (built, dev): `--job trend`, its own Cloud Run job and scheduler** | Replaces the former LSTM predictive path (dropped 2026-08-10): reads the member's multi-horizon rollups + R1 baselines, computes trend features deterministically (moving averages, slopes, deviations — .NET, no ML), injects the **pinned clinical reference-range table**, and asks MedGemma for a family-facing trend narrative feeding digests/insights. No risk scores, no per-user models. **Built 2026-09-20.** `TrendFeatureCalculator` (Application, package-free) computes a 7-day moving average, a least-squares slope over 28 days and the recent average against each of the 30/60/90-day baselines the Worker already writes — the design named "multi-horizon rollups", but the daily `ActivityLog` *is* the daily horizon and the longer baselines are the long ones, so no rollup table was needed. `PinnedReferenceTable` assembles the published figures from `HealthReferenceRanges` and `WellnessGuidelines` rather than writing new ones, and **deliberately omits resting HR by age/sex**: this codebase has no sourced table for it, and composing one from memory is the failure pinning the table exists to prevent. `TrendInterpretationService` stores the result as `MemberInsight` scope `Trend`. Cold start: under 30 days of readings it declines outright — that is the learning state, not a trajectory. Its prompt version folds in the pinned table's, so a narrative written against superseded figures does not outlive them. **Three horizons since 2026-09-21** — see "Trend horizons" below: the rolling read above, plus `TrendWeekly` and `TrendMonthly`, which are journal-aligned and ride the digest pass instead of this job |
| `StatisticalJudgement` | Cloud Scheduler | Every 5 min, inside the assessor job | **Built:** `StatisticalAlertService`, run by `carditrack-<env>-pipeline-jobs-assessor` after the real-time assessment and before its digest pass. The eleven R1 rules (`StatisticalAlertRules`, Application, I/O-free — nine comparative rules against the established 30-day baseline, plus the two measured rhythm rules `irregular_rhythm` and `ecg_afib` since 2026-09-22, which relay a device classification and need no baseline) compute *findings* — what was measured, what is usual, the yardstick crossed — and every finding that survives cooldown and same-day dedup goes to MedGemma (`CARDITRACK_STATISTICAL_JUDGEMENT_PROMPT`, clinical half) in one call per member. The model returns a severity and a clinical read per finding, matched by rule; code maps the severity strictly, and the reads that clear Yellow go in one further call to the **Rewrite** slot, which writes the headline and message a caregiver sees from them and nothing else. Code then guards the register, writes the alert, regenerates the status line and POSTs the alert id to the internal enqueue endpoint. The tone block moved to the rewrite half on 2026-09-22, for the reason `StatusLineGenerationService` gives: a medically-tuned model told it is writing for a family spends the decode on wellness copy. **The rules are an input provider; the inference is the model's** — a constant never pages a family. Fail closed: an unmapped or missing verdict, a blank clinical read, a model failure on either slot, a rewrite that drops an entry, or rejected copy raises nothing and is re-judged next pass. Cost: one private-slot call per member per pass in which a finding survives cooldown and dedup, none for a member with nothing off, and **one further Rewrite-slot call** in the passes where at least one read clears Yellow — a pass whose findings are all judged benign still costs the one call it always did. A raised alert dedups its rule for the local day, so a finding that becomes an alert costs one call; a finding the model judges `low` **is** now remembered, in `BenignJudgements`, keyed on a hash of the finding itself (`StatisticalAlertRules.JudgementFingerprint`: rule, observation and figures) rather than on the day. Readings that move produce a different fingerprint and the finding is judged again, so a repair sync or a late-arriving night cannot hide behind this morning's verdict; readings that do not move cost one call instead of 288. No rule is exempt and none needs to be — `no_morning_activity` names the clock in its observation, so it re-judges every pass on its own, and the two measured rules carry the device's counts, so a notification arriving this afternoon is a new question |
| `DigestGenerator` | Cloud Scheduler | **Built (dev):** half-hourly scheduler, plus a second pass at the end of each assessor run (every 5 min). Regenerates for **whichever members' readings have moved since their last summary** — members whose data has not changed, and members summarised within the last 20 minutes, are skipped before any model call. The two gates decouple the cadence from the *inference* bill: the jobs can run often enough to catch a quiet member up quickly without regenerating a continuously-uploading one on every pass. They do not make the cadence cost-free, though — the shared MedGemma GPU service scales to zero, so a pass that finds any work at all pays a cold start at the full allocation. **The floor is waived** when an alert was raised or resolved, when the latest real-time window is yellow-or-above or an SSA jump (≥3 typical jitters from trend), or when new daily readings diverge from the 30-day baseline or jumped more than 30% from yesterday (SpO₂: 3 points). Those are the changes that rewrite what the summary should say; making a caregiver wait out the floor to read them — including a medium observation that has not (yet) become an alert — would be the floor working against its own purpose. Ordinary new readings still ride the cycle | Summarises the member's local day in progress (their anchor timezone is the earliest-linked caregiver's `User.TimeZoneId`) → calls MedGemma (`CARDITRACK_FAMILY_DIGEST_PROMPT`, returning a short `headline`, the summary text, one `suggestion` and an `urgency`) → **appends** to the partitioned `DigestEntries` table, whose key carries `GeneratedAtUtc` so every recomputation is kept as history (**7-month** retention by partition drop); read via `GET /api/v1/insights/members/{id}/digest` (current) and `GET /api/v1/insights/members/{id}/digests` (history). The same call may also propose **one short question to the family** when the readings would be clearer for an answer; it is stored as a `MemberQuestionnaire` (see `docs/execution/backend/api/questionnaires.md`) under noise gates — one open question per member, seven days since the last ask (standing/`Permanent` questions skip the floor; a gap-backed ask from an unresolved alert or Yellow+ observation may fire with a 12-hour ceiling; dismissed and answered-permanent never re-asked). Push infrastructure is built (FCM HTTP v1 relay with APNs passthrough); the digest's own push wiring is what remains. The family audience is the only audience — wearers never log in |
...
| Model name | `medgemma-1.5-4b-it:q4_k_m` — what `ollama create` registers, named in `src/Infrastructure/MedGemma/.model-version`; `docker-compose.yml` and `AI__Private__Model` must both match, and the build asserts `ollama list` shows exactly it |
| Model pin | `src/Infrastructure/MedGemma/weights.sha256` — the sha256 of each GGUF, confirmed against both the upstream LFS object and the layer digests of the tag previously served. Checked by the fetch script, again by the deploy workflow after downloading from our bucket, and a third time inside the Dockerfile before Ollama sees the bytes. No stage pulls from a registry: Ollama's CVE-2026-85180 fix refuses the cross-host redirect the old tag's blobs arrived by |
| Model source | `weights.env` names the upstream repo and commit; `vendor-medgemma-weights.yml` fetches once and keeps the files in `carditrack-common-model-weights` under a content-addressed path (`<model>/<sha256>/<file>`), versioned and never lifecycle-deleted |
| Modelfile | `src/Infrastructure/MedGemma/Modelfile` — template and params in git, byte-for-byte what the previous tag served (template layer `e0a42594…`), so a review sees any change to how the model is prompted |
| Model aliases | `src/Infrastructure/MedGemma/.model-aliases` — extra names the image answers to during a rename, so it can be deployed before the environment stack flips callers. Empty when no rename is in flight |
| Base image | `ollama/ollama:0.34.2`, pinned by digest — the version tag is there to be readable, the digest is what pins |
| Sampling | `temperature` 0.1, sent as a request option (`AI:Private:Temperature`) and also pinned in the Modelfile. Not inherited from anyone's upload |
...
| `VertexGemini` | `HttpClient` (`generateContent`) | `https://{Location}-aiplatform.googleapis.com` — derived, EU allowlist | No API key: OAuth access token via ADC (`VertexAccessTokenHandler`), IAM `roles/aiplatform.user`. Structured output via `responseJsonSchema`; `usageMetadata` parsed; MedGemmaClient-grade telemetry/no-logging, and its retry contract but for the client-side timeout, which is retried once here rather than being terminal — a managed endpoint holds nothing of ours when a request is abandoned (`VertexAiClient`) |
...
| Activity level | `activity-level` — `list` (intervals) | Per interval | **Built 2026-08-22.** Read as intervals rather than as the rollup, for the one figure a daily total cannot carry: the *longest unbroken* sedentary stretch. Six hours split into twelve half-hours and six hours taken at once sum the same and are not the same day. Clipped to the night's sleep session, and left null when that session is unknown — an unclipped figure is a reading about sleep wearing a daytime rule's name |
| ECG classification | `electrocardiogram` — `list` | Per wearer-initiated reading | **Built 2026-09-22**, behind `googlehealth.ecg.readonly`. The **classification only** — `waveformSamples` is excluded by a `fields` partial-response selector and never crosses the wire. Thirty seconds of lead-I voltages at 500 Hz is 15,000 integers of diagnostic PHI per reading that CardiTrack neither stores nor shows, so fetching it would mean moving it through the process only to discard it. Only `ATRIAL_FIBRILLATION` counts as a finding; `INCONCLUSIVE*`, `UNREADABLE` and `NOT_ANALYZED` all mean the device declined to judge. Its filter grammar is the one exception to the civil-day pattern — a single `>=` on `interval.start_time` with no upper bound and no civil sibling — so the read opens a UTC day early and settles each reading's civil day from its own `civilStartTime` |
| Irregular-rhythm notification | `irregular-rhythm-notification` — `list` | Per notification (passive) | **Built 2026-09-22**, behind `googlehealth.irn.readonly`. The device's own optical AFib screening, and the only passive rhythm signal available: ECG needs the wearer to take a reading, which the product never asks of them. The count is of notification records, not of the `alertWindows` inside them — a notification is one thing the wearer was told. The windows and their beats land in `RhythmEpisodes` (below) |

> **HRV needs no new consent**: it reads under `health_metrics_and_measurements`, which every connection already holds. Weight and blood glucose were built alongside it and remain **parked before merge**, because both depend on a connected scale or meter most wearers do not own; their implementation is on record in commit `dce291d`.
>
> **ECG and IRN shipped 2026-09-22** and are inert until verification covers them. Both need their own scopes (`googlehealth.ecg.readonly`, `googlehealth.irn.readonly`), both are Google-classed SaMD features, and both are in the restricted-scope verification track of issue #39 — added to that submission rather than left for a later one, because verification is per-project and a scope added after approval throws the project back into review. A connection without them costs no request: the caller checks the stored scope list first, and every rhythm count stays null, which the product reads as "cannot see this wearer's rhythm data" and never as "nothing happened".
>
> **Where the two scopes are configured, and where they are deliberately not.** `dev.tfvars` carries all six scopes for the Google Health provider (the four verified ones plus `ecg.readonly`/`irn.readonly`) — safe there because the dev project is never submitted for verification and runs in Testing mode, where an allow-listed tester can grant a restricted scope directly. `prod.tfvars` carries only the four. `CardiTrack.API/appsettings.json` — the file baked into every environment's image, since there is no `appsettings.Production.json` split — also carries only the four, matching prod rather than dev. This was gotten wrong once: an earlier revision added all six to `appsettings.json` for local-dev convenience, on the reasoning that a developer running the API directly (no Terraform env vars) should see the rhythm scopes too. That broke production. ASP.NET Core's configuration system merges array-indexed keys across providers rather than replacing the array wholesale — an environment-variable provider that sets `DeviceProviders__0__Scopes__0..3` does not delete indices `4` and `5` a lower-priority JSON provider already set, so prod would have ended up requesting two unverified restricted scopes and had its entire consent refused by Google, taking the four working scopes down with it. `appsettings.json` must therefore never carry more scopes than the environment with the fewest of them is meant to have; a developer who wants the rhythm scopes locally supplies them the same way `ClientId`/`ClientSecret` are supplied — as local environment variables or user secrets, never checked in.

### Beat-level rhythm data — what is and is not possible

`HeartBeat.beatsPerMinute` is documented in the v4 discovery document as `60000 / rr`, where rr is the interbeat interval in milliseconds, so true RR intervals are recoverable. **They are only ever served inside an irregular-rhythm notification's `alertWindows`**, and `AlertWindow`'s own description settles what that means:

> the current version of the algorithm will only produce alerts if all windows are positive. So anything returned from the API will always have the positive bit set to true

Negative and inconclusive windows are never saved, and there is no standalone continuous beat-to-beat data type — `HeartBeat` is referenced by exactly one schema, `AlertWindow`, which is referenced by exactly one type. Three consequences the product has to hold:

- **CardiTrack cannot detect arrhythmia independently.** Beats arrive only after Google's own algorithm has already made a finding. A rule treating an absence of episodes as an absence of arrhythmia would be reading silence as reassurance.
- **What the beats are for is evidence, not detection**: episode duration, beat count and RMSSD, so a caregiver and the clinician they take it to see more than "a notification happened".
- **`AlertWindow.positive` is true on every row served.** It is stored faithfully, but nothing branches on it — a filter that has never once excluded a row is a filter nobody is testing.

Beats are stored in `RhythmEpisodes`, day-partitioned on the window start at **90-day** retention (the same as `RealtimeAssessments` and the granular minutes), with RR recovered from `beatsPerMinute` rather than approximated from timestamps, and RMSSD computed at ingestion. Per-beat offsets are kept alongside the intervals because the two are independent measurements: offsets come from each beat's `physicalTime`, intervals from its `beatsPerMinute`, and where beats are missing the offsets show the gap and the intervals cannot.

`IrnProfile` (`GET /v4/users/me/irnProfile`) reports whether the wearer completed IRN setup and is currently enrolled, recorded per connection. This exists because the alternative is a silence nobody can read: a wearer who never finished setup raises no notifications at all, which on every screen looks exactly like a wearer whose heart is behaving.
...
**Live prompt:** `CARDITRACK_REALTIME_ASSESSMENT_PROMPT` in `RealtimeAssessmentService` — since 2026-09-22 a **clinical read**: `WearableClinicalOpening` and the full `ContextGuardrail` — this purpose's context carries the family's questionnaire answers, so the notes-only variant would leave that untrusted section unframed, then an hour of SSA yardsticks, then what the readings show in clinical terms plus a strict `critical` / `high` / `medium` / `low` severity token. Four of the five consumers of `ModelOutput` are other prompts (daybook, member chat, the digest's monitoring context, the status line's clinical half), so the tone block was throttling all of them; the fifth is the heart-rate alert, which now rewrites the read on the **Rewrite** slot before a caregiver sees it. That rewrite is **fail safe, not fail closed** — a failure still raises the alert carrying `NonClinicalObservation`, because silence about a heart rate the model has just called urgent is the one failure this path must never produce. The sketch that used to sit here (`[CARDITRACK_SYSTEM_PROMPT]`, "medical AI assistant", "flag for review", "clinical attention") is not sent. MedGemma copies sample phrases verbatim, so the live instructions name the SSA threshold (`scores under 3 are ordinary variation`) and tell the model to read activity and conditions in the data, without illustrating exercise, heat, or poor air.
...
Handed a `CardiTrackCardiMember` placeholder and told to write with it, a 4B model repeats the placeholder in every sentence of a six-sentence summary. When sex is known, that is the wrong shape — a case file about a subject, not one person telling another how someone is doing — so the rule is he or she after at most one name. When sex is not stated, repeating the name is the lesser wrong: "they" is a stranger's word for a family reading about one specific person, and every member created before M1-04 asked for sex sits at "not stated". The line used to open "Name them once": after Tone has just named the family member as the reader, "them" attaches to the reader, and most of the prompts that carry this rule never send a name at all, so it was also an instruction to invent. The token itself stays out of this line because alert and assessor copy is stored without resolving it; "they" remains only for that nameless, sex-not-stated case.

It follows `Tone` in **every prompt that writes prose a caregiver reads** — the digest rewrite, the assessor, and the alert/baseline/learning/provisional insights. The status line's *clinical* half is a MedGemma read and does not carry it (or this pronoun rule): that throttle is what left the hero writing wellness copy. Its *rewrite* half carries `PronounsByToken` like the other Rewrite-slot briefs. `MedicalPromptToneTests` pins both halves.

### The pronoun token (built today)

The rule above can only be followed by a prompt that is shown the sex line, and four of the prompts carrying it never were. The digest rewrite, the Advise rewrite, the member-chat rewrite and the status-line rewrite all run on the Rewrite slot, which receives a `DeidentifiedFindings` and nothing else — no member context, so no `Sex:` line, so no answer to the rule's first clause. They answered it anyway. On 2026-09-11 a member's summary card said "he" and "his" while the suggestion card beneath it said "them", about the same person on the same screen; the "he" matched the record, but nothing in the chain that wrote it had been told, and for the population still at "not stated" the same guess is a coin toss about someone's father or mother.

So those four briefs carry `MedicalPromptBlocks.PronounsByToken` instead, and the pronoun is settled the way the name already is — asked for as a token, substituted in code:
...
### Trend interpretation pipeline (built)

```
Cloud SQL (ActivityLogs + PatternBaselines)
  ↓
Trend-feature computation (TrendFeatureCalculator, .NET — no ML)
  — Computes: resting HR / steps / sleep / active minutes / overnight HRV /
...
Routing: narrative feeds the family digest and the insights API
  — built: the digest prompt carries it as a labelled "longer view" section
    (context only, never a draft to rewrite), and GET /insights/members/{id}/trend
    serves it. Deliberately NOT the CardiJournal books: each is a raw reassessment
    of its own period and never reads another narrative.
...
Three, sharing one service, one feature calculator and one pinned table. What differs is the span the figures are drawn over, the opening sentence of the brief, and the scope the row lands under.

| Horizon | Scope | Due | Covers | Host |
|---|---|---|---|---|
...
**Guards.** Each journal horizon refuses a period measured on fewer days than its book's threshold — four of seven, fourteen of a month, whatever that month's length — checked before any model call. The threshold is the book's; what counts toward it is not, and deliberately: a book describes whatever the period recorded, so a day carrying only distance or SpO2 counts for it, while a trend reads six series and can plot none of those. A member with four distance-only days therefore gets their Weekbook and no weekly trend, which is correct — there was nothing to trend. That is a separate question from the cold start, which asks whether the member has a learned normal at all and is the same 30 days at every horizon: the horizon changes what is described, never how much history it takes before anything is. Written once per member per local day, anchored to the start of their own local day rather than a fixed interval, so a 25-hour fall-back day cannot admit a second narrative of the same period.

**Overlapping executions.** The digest job is scheduled every thirty minutes (`pipeline_jobs_schedule`) against a Cloud Run task timeout of an hour, so a slow pass is still running when the next execution starts. Both then read the same member, both see no book or narrative for the period, and both call the model. The unique indexes keep the stored data right either way — one insert is rejected, or two executions write the same row from the same readings — so what was being lost was inference, on a service whose measured cost profile says cadence is the only lever.

Every once-per-period writer on that pass therefore takes a **claim** before it calls the model: the three CardiJournal books and both journal-aligned trend narratives, through `GenerationLeases` (`DigestGenerationService.UnderClaimAsync`). One row per member per `GenerationWork`, taken by a single `INSERT ... ON CONFLICT DO UPDATE ... WHERE` whose predicate admits the update only when the existing lease has lapsed — so the exclusivity is Postgres', not the caller's, and two executions racing it cannot both come away holding the period. A loser skips that member rather than queueing: the other execution is doing the identical work.

It is a lease and not a lock. The claim carries a twenty-minute expiry (`GenerationLeaseTerm.Default`, derived from the 900-second client budget a single medical call may run to), so an execution killed mid-generation by a deploy, an OOM or the job's own timeout costs the member one period's delay rather than blocking that period for good. It is released in a `finally`, so a failed generation is retried on the next pass instead of waiting the lease out.

`TryClaimAsync` returns a **claim id**, and the release is keyed on it rather than on (member, work). That is fencing, not bookkeeping: a generation that overruns its lease has already been taken over by a later execution, and a release keyed on the pair would delete the successor's live row and let a third execution in while the second was still working. A takeover rewrites the row's `Id`, so the displaced holder's release matches nothing. Renewal is deliberately not implemented — with fencing an overrun is merely wasteful rather than harmful, since the successor's write lands and the overrunning holder's is absorbed by the same unique indexes that made the race survivable in the first place. The row is keyed without the period it holds, which bounds the table at members × works and is why nothing has to sweep it. Deliberately not the Worker's `AdvisoryLock`, whose own remarks say it is "restraint, not correctness" and that work needing exclusivity "wants a claim it can lose safely, not this" — and which would hold a database session open for the whole of a model call.
...
**Read surface.** `GET /api/v1/insights/members/{id}/trend?horizon=rolling|weekly|monthly`, defaulting to `rolling` — what the endpoint returned before the horizons existed, so existing callers are unaffected. An unrecognised value is a 400 rather than a silent fallback.
...
**Trend prompt (built — `TrendInterpretationService`).** It uses the same shared blocks as every other family-facing generation (`Tone`, `Pronouns`, `CaregiverRegister`, `ContextGuardrail`). Sample hedges and example outputs are not listed: MedGemma copies them verbatim.

The brief is one shared body behind a per-horizon opening sentence, which names the stretch being read — "a month of one person's wearable readings", "the week that has just ended", "the month that has just ended". The stretch is stated rather than left to be inferred from the rendered window header: a model handed seven days of averages under a brief that says "a month" writes about a month, and the noun in the first sentence is the one that reaches the caregiver. Three openings means three fixed prefixes, each still fixed.

The body asks the model to say what the figures say and nothing else: never work out a percentage, a difference or a direction, never introduce a number that is not in front of it. The one comparison it may make is placing a figure against a range printed below it, because that is reading two given numbers rather than calculating a third. It is given two yardsticks with different jobs — the member's own usual, which says whether this is a change *for them*, and the pinned published ranges, which say whether it sits where the bodies that publish guidance say it should — and asked to use both, since either alone leaves the family to guess the other.

> The standing refusals are in the brief and pinned by test: never a condition, a diagnosis or a treatment; never a score, a probability, a risk level or a prediction of what will happen next. Saying a figure sits outside a published range is a fact about the figure and is wanted; saying what it might lead to, how likely that is, or what it puts them at risk of is none of those and must not appear.

> `BriefVersion` is packed with `PinnedReferenceTable.Version` into one stamp, so a row written against superseded wording *or* superseded figures is due for regeneration whatever its age.
...
|--------|---------|----------|---------|
| `CARDITRACK_SYSTEM_PROMPT` | Every 5 min | Internal (clinical review queue) | **Superseded** by live `CARDITRACK_REALTIME_ASSESSMENT_PROMPT` (`RealtimeAssessmentService`), which since 2026-09-22 is itself a **clinical read**: SSA yardsticks in, what the readings show in clinical terms plus a strict severity token out. Four of its five consumers are other prompts; the fifth is the heart-rate alert, which rewrites it on the **Rewrite** slot before a caregiver sees it |
| `CARDITRACK_LEARNING_PROMPT` (clinical half) | On request, before any baseline | The shared baseline rewrite — no caregiver reads it | What has been observed so far, before any baseline exists. Split off the tone 2026-09-23; the card a family reads is written by `BaselineRewriteInstructions` on the **Rewrite** slot |
| `CARDITRACK_PROVISIONAL_PROMPT` (clinical half) | On request, while only a 7/14-day baseline exists | The shared baseline rewrite — no caregiver reads it | Early impressions against a provisional baseline. Split off the tone 2026-09-23, sharing one rewrite half with the learning and baseline briefs |
| `CARDITRACK_FAMILY_PROMPT` | On high/critical events | — | **Superseded** by live `CARDITRACK_ALERT_PROMPT` (`HealthInsightService`), split into the two halves below |
| `CARDITRACK_ALERT_PROMPT` (clinical half) | Once per alert, on the pass that raised it | The alert rewrite below — no caregiver reads it | What the readings behind the alert show, in clinical terms, and the basis for an action. Split off the tone 2026-09-23 |
| `CARDITRACK_ALERT_PROMPT` (rewrite half) | Same pass, only when the clinical read came back | Family members (as the alert detail card) | `AlertRewriteInstructions` on the **Rewrite** slot, receiving that read as a `DeidentifiedFindings` and nothing else. The stored copy is held to the read: an explanation naming a reading the read did not is discarded rather than shown |
| `CARDITRACK_STATISTICAL_JUDGEMENT_PROMPT` (clinical half) | Every assessor run, for members with a finding that survived cooldown and dedup | The rewrite half below — no caregiver reads it | `WearableClinicalOpening` and the full `ContextGuardrail` — this purpose's context carries the family's questionnaire answers, so the notes-only variant would leave that untrusted section unframed, then the findings as JSON — rule, observation with figures and the yardstick crossed, the stored metric values — and, per finding, a strict severity token plus an ≤80-word clinical read naming the mechanism where there is one. **Built (2026-09-19)**, split off the tone **(2026-09-22)** in `StatisticalAlertService` |
| `CARDITRACK_STATISTICAL_JUDGEMENT_PROMPT` (rewrite half) | Same pass, once per member, only when a read survived the clinical half | Family members (as alerts) | Tone, `PronounsByToken` and `CaregiverRegister`, then the clinical reads and nothing else — a `DeidentifiedFindings`, DPIA row A20's compile-time boundary. Returns a two-to-six-word headline and 1–3 caregiver-actionable sentences per read that name no day or clock time. Runs on the **Rewrite** slot |
| `CARDITRACK_FAMILY_DIGEST_PROMPT` | Whenever the member's readings have moved past their last summary, or samples indicate a problem / diverge from baseline / jumped from yesterday, or their alert state changes (half-hourly job, and immediately after each assessor pass) | The family-digest rewrite — no caregiver reads it (since #507) | Headline, 2–5 sentence interpretation of the local day so far (usual pattern + steps walked as the movement yardstick for vitals; family answers used to read the day, never retold; a computed still-day/raised-rate pairing is injected when it fires), one supportive suggestion in plain language, and optionally one question for the family — **built today** (append-only store with history + API read; push pending). The suggestion must answer these readings and must never name or guess at a medical condition; naming a possible diagnosis stays the job of the alert/severity pipeline below, which never names a condition either — it prompts "reach out now" and surfaces one-tap contact, deliberately without a diagnostic guess. |
| ~~`CARDITRACK_DIGEST_PROMPT`~~ | — | ~~Wearer~~ | **Descoped 2026-08-10** — wearers never log in; self-monitoring is not the product |
| `CARDITRACK_TREND_PROMPT` | Daily for the rolling read (`--job trend`); on the member's own week start and the 1st for the weekly and monthly horizons (`--job digest`) | The trend rewrite — no caregiver reads it | Trend narrative over computed features + pinned reference ranges — replaces `CARDITRACK_PREDICT_PROMPT`, which died with the LSTM's risk scores. Since 2026-09-22 a **clinical read**: the figure-reading discipline stays here and the caregiver register moved to a rewrite half on the **Rewrite** slot, which writes the account a family reads. No sample hedges or example outputs. One shared body behind a per-horizon opening |

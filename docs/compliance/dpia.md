...
| A10 | **Baseline calculation** — daily job deriving a behavioural profile per member from 7/14 (provisional) and 30/60/90 days of `ActivityLog` history: **arithmetic mean and sample σ** for steps and resting heart rate, sleep duration and efficiency, **typical bedtime and wake time** (circular mean), and a steps-by-weekday profile. Written to `PatternBaselines`; no external recipient. **Limitation (not a defect of the job):** mean/σ are not robust — a single atypical day inflates σ, which *widens* the HR alert margin (`max(2σ, 5 bpm)`) and can hide a later genuine elevation. Median/MAD are implemented in-process (`IDescriptiveStatistics`) but **not persisted or used for alerts** — see [mathnet_numerics.md](../technical/mathnet_numerics.md) G1 | **H** (behavioural profile) → stays in-project | `src/Worker/CardiTrack.Worker/Workers/BaselineCalculationWorker.cs`; `src/Core/CardiTrack.Application/Services/BaselineCalculator.cs` |

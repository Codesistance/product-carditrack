...
| A12 | **Webhook notification receipt** (dev) — public-ingress Cloud Run service authenticating Google Health webhook notifications by shared secret and buffering the **raw, unparsed** payload in region-pinned Pub/Sub for the (unbuilt) aggregator. Notifications carry at most the pseudonymous `healthUserId`, a data type name and a changed interval — no readings; the process holds no database, AI or user credentials | *(h)* pseudonymous device-scoped id + metric type in transit | `src/Pipeline/CardiTrack.HealthWebhookReceiver/Program.cs`; `infrastructure/deployments/pubsub.tf` |

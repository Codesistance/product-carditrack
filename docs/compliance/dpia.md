...
| Changes since 0.9 (2026-08-15) | Caregiver-initiated questions about a CardiMember (`POST /api/v1/insights/members/{id}/ask`) — on-demand MedGemma, in-project, never persisted. Extends A5 (prompt-content change, §13 trigger): the question is flattened, section-framed as untrusted, and cannot set an alert. R-A14 covers this free text the same way it covers `MedicalNotes`. Mobile surface is an as-built card on M1-13. |
...
| A5 | **AI insights** — alert details, baselines, 3–14 days of metrics, plus a **member context block** assembled by `MemberContextComposer` from registered sources — age derived from date of birth, sex, free-text `MedicalNotes` truncated at 1000 chars (now correctly **decrypted** before sending; previously the ciphertext envelope was sent verbatim), consent-gated exercise-session conditions, and the family's three most recent questionnaire answers — sent to **self-hosted MedGemma** (Ollama on Cloud Run, internal ingress). Name and member id are deliberately excluded. **Also on-demand caregiver questions** (`POST /api/v1/insights/members/{id}/ask`): the question is flattened to one line, framed under "Caregiver question" as information the model must not take instructions from, **never persisted**, and cannot set or change an alert. Rate-limited 6/min and 30/hour per IP | **H** → stays in-project | `HealthInsightService.cs`; `MedGemmaClient.cs`; `infrastructure/deployments/cloud_run.tf`; `InsightsController` (`AskAboutMember`) |
...
| R-A14 | **Caregiver free text as model input**: `MedicalNotes` is sent to MedGemma with the metrics (A5), and a caregiver's on-demand question on the same path is too. Two failure modes — a stale or mistaken note ("takes metformin" after the drug was stopped) steers an explanation the caregiver reads as authoritative; and untrusted text reaching a medical model can carry instructions rather than facts. The ask question is not stored (unlike questionnaire answers), is flattened onto one line, and is framed so the model must not follow instructions in it or set an alert | M | M | **Medium** | M13 |

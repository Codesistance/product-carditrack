...
|---|---|---|---|
| A1 | **Wearable ingestion** — 10-min polling of Google Health API (restricted scopes: activity, health metrics, sleep) for the previous day's rollup; upsert into `ActivityLog`. Includes a one-time **historical import** per connection: up to 90 days of pre-connection history, fetched in chunks after connect, under the same scopes and consent as the ongoing pull | Daily activity, heart rate, sleep per member — incl. up to 90 days predating the connection | `WearableSyncWorker.cs`; `src/Infrastructure/CardiTrack.Infrastructure/Services/DeviceSyncService.cs`; `.../ExternalClients/FitbitApiClient.cs` |

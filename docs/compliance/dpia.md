...
|---|---|---|---|
| A1 | **Wearable ingestion** — 10-min polling of Google Health API (restricted scopes: activity, health metrics, sleep) for the previous day's rollup; upsert into `ActivityLog`. Includes a one-time **historical import** per connection (up to 90 days of pre-connection history, fetched in chunks after connect, under the same scopes and consent as the ongoing pull) and, on the Worker cadence, **minute-grain series** (heart rate, steps, active-zone minutes, SpO2) into the partitioned `GranularMetricHours`/`MetricRollupsHourly` tables — retention 90 days / 13 months by partition drop, enforced by `PartitionMaintenanceWorker` | Daily activity, heart rate, sleep per member — incl. up to 90 days predating the connection, and minute-grain heart-rate/SpO2/movement series (90-day retention) | `WearableSyncWorker.cs`; `src/Infrastructure/CardiTrack.Infrastructure/Services/DeviceSyncService.cs`; `.../Services/GranularIngestionService.cs`; `.../ExternalClients/FitbitApiClient.cs` |

...
| A11 | **Family summary** — hourly Cloud Run job (AI pipeline, dev) generating a plain-language summary (headline + 2–4 sentences) per member via **in-project MedGemma** (prompt carries age, sex, medical notes, daily readings — never name or id), recomputed whenever the member's readings have moved since their last summary rather than once each morning; every generation is retained, so a member's summaries form a history; stored in partitioned `DigestEntries`, 12-month retention by partition drop; read relationship-scoped via the insights API. The job is wired without any public-provider key, so it cannot send data off-estate | **H** (derived narrative about health) → stays in-project | `src/Pipeline/CardiTrack.PipelineJobs/Program.cs`; `src/Infrastructure/CardiTrack.Infrastructure/Services/DigestGenerationService.cs` |

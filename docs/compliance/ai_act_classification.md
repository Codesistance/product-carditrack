...
| A5 | AI insights — the model's narrative behind an alert (the second card on alert detail) and the baseline insights on member detail, generated by `HealthInsightService` from alert details, baselines, recent metrics and the member-context block | MedGemma | **dev only** (prod grants no MedGemma invoker) | Yes — generated text shown to caregivers |

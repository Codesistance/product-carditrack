...
| A15 (+ A26) | Statistical findings judgement — fourteen deterministic rules produce findings: nine *comparative* rules against the member's 30-day baseline, two *measured* rhythm rules (`irregular_rhythm`, `ecg_afib`) carrying a finding the wearer's device already classified (A26), and since 2026-09-25 three *published-range* rules (`sleep_outside_range`, `resting_hr_outside_range`, `spo2_below_range`) against the NSF, AHA and WHO ranges; the last five are deliberately not gated on a baseline. **Since 2026-09-19 MedGemma returns the severity, headline and message** for every finding, of all three kinds, and that verdict becomes the alert row | MedGemma | **dev only** (moved from the Worker to the pipeline's `assess` job). The two A26 rules are **inert in every environment**: neither rhythm scope is granted yet (restricted-scope verification, issue #39), so they produce no findings today | Yes — the model is now in the severity path of the main producer, not a sidecar |
| A5 | AI insights — the model's narrative behind an alert (the second card on alert detail) and the baseline insights on member detail, generated by `HealthInsightService`; and the rolling, weekly and monthly **trend narratives** ("Trends to keep an eye on", "The longer view") generated by `TrendInterpretationService` and served as `TrendInsightResponse` — all stored in `MemberInsights`, all from readings, baselines and the member-context block | MedGemma | **dev only** (prod grants no MedGemma invoker) | Yes — generated text shown to caregivers |
...
| A11 | Family summary (headline + body) and the persisted CardiJournal outputs — Daybook, Weekbook and Monthbook entries with their headlines, summaries and suggestions — all written by `DigestGenerationService` in the two-slot clinical read + rewrite split | MedGemma + Gemini | dev | Yes — generated text shown to caregivers |
...
This route opens only through DPIA **OI-2** (medical-device classification, EU MDR Rule 11),
and OI-2 alone does not close it. Three conditions must all hold: (i) OI-2 concludes CardiTrack
is a device; (ii) that device is required under the MDR to undergo a **third-party conformity
assessment** — a self-declared Class I device does not satisfy Art. 6(1)(b), while Class IIa and
above do, as do the Class I sterile, measuring and reusable-surgical variants that need a
notified body, so the test is the conformity route OI-2 lands on, not a class threshold; and
(iii) the AI system is the
product or a *safety component* of it (Art. 3(14) — a component whose failure endangers health
or safety). Condition (iii) is likely met if the model-routed early warning is the very
function that makes CardiTrack a device, and must be argued separately if the device claim
rests on something else. Only when all three hold is the severity-routing AI Annex I high-risk,
with an application date of 2 August 2028 and a conformity route that runs through the device's
own notified body. If OI-2 concludes it is not a device (general wellness positioning, "not a
medical device", `docs/solution_manifest.md`), Annex I does not apply. The 2026-09-22 rhythm ingestion (DPIA A26 — ECG classifications and
...
**Position recorded:** *Annex I is closed unless OI-2 concludes "device"; if it does, conditions
(ii) and (iii) are assessed then, not assumed. No separate decision is taken here.*
...
surfaces (A5, A11, A19, A20, A22, A24, A6) generate text for a caregiver and sit in no Annex III
...
- the model deciding *what is done* rather than *who is told* — e.g. SMS dispatch to a
  non-caregiver, automatic escalation to anyone **outside the member's registered caregiver
  set** (today's ladder fans out only within it, DPIA A18), or any effect on service, price or
...
|---|---|---|
| Member chat sheet | **Present (2026-09-25), as a one-time notice.** The first time a caregiver opens the chat on a phone — and ahead of a send, should it somehow not have been seen — a notice says "Answers here are written by CardiTrack's AI assistant, not a clinician", with one button, "Got it". Remembered per caregiver on the phone and cleared at sign-out, so the next person on the same phone is told afresh. Art. 50(1) asks for the information at the latest at the first interaction, which this meets. It replaced, the same day, a fixed line under the sheet header that was on screen in every mode; the owner found it crowded the header. Was: none — the empty state said "Ask anything about their readings" and the reply bubble carried a bot mark but no words | `src/Presentation/CardiTrack.Mobile.Core/Chat/AiChatNotice.cs`; `MemberChatPage.xaml.cs` (`EnsureAiNoticeSeenAsync`) |
...
| PDF report narrative and journal entries; chat-transcript export (**PDF**) | **Present** — an "AI" mark and a line under the narrative ("Written by CardiTrack's AI assistant from the readings in this document. It is not a clinical assessment.") and, since 2026-09-26, once under the Journals heading rather than on every card ("Every entry below was written by CardiTrack's AI assistant. Not a clinical assessment."); the transcript's provenance banner and its every-page footer "AI-generated answers · Not a clinical assessment" | `PdfReportRenderer.cs`, `ChatTranscriptDocument.cs` |

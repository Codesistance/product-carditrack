...
| Established window only | Statistical alerts fetch the **30-day** baseline. 7- and 14-day *provisional* rows colour the dashboard and never page. The two **measured** rhythm rules in §2 are the deliberate exception: they need no baseline because the finding is the device's, not a comparison. So are the three **published-range** rules (`sleep_outside_range`, `resting_hr_outside_range`, `spo2_below_range`, 2026-09-25): they compare against the NSF, AHA and WHO ranges, not against the member's usual — ≥3 of the last 5 days outside, one alert per stretch, severity the model's | `StatisticalAlertService` |
...
**Long daytime rest counts only worn time.** Stillness alone does not fire it. `activity-level` is the one input here the Google Health API does not document as worn-only (`sedentary-period` is defined as "not moving while wearing the device"; `activity-level` carries no such qualifier), so a watch left on its charger could otherwise read as an afternoon without moving. The sedentary intervals are therefore clipped to the time the watch had a heart rate, read as five-minute heart-rate rollup windows from wearable sources. One empty window is treated as a loose band and bridged; two or more in a row (ten minutes) split the stretch. A day whose heart rate cannot be read at all reports no stretch, which the rule treats as "could not judge" — never an unchecked stretch in its place. The cost is deliberate: a band that drops heart rate for ten minutes while the wearer sits still splits a real stretch, which errs toward staying quiet on a yellow alert.

...
| Day reviews (once-daily account of a finished day, clinical register) | R2 | R2 | — | — | 🔶 **Shipped in dev ahead of R2** — `DigestAudience.DayReview` entries written by the digest job at 02:00 in the member's local time, one per day, never recomputed; read via `?audience=day-review` on the insights digest endpoints and listed on the mobile **Summaries** tab (which replaced the Family stub). Register allows a precise term where it explains itself; conditions, diagnoses and treatment refused in code (`DayReviewPrompt`) |

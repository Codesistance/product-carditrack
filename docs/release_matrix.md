...
| Statistical alerts (all 5 launch types) + acknowledgment/notes | R1 | R1 | R1 | — | 🔶 **Generation + read path shipped.** `AlertGenerationWorker` evaluates all five rules hourly off `PatternBaselines`; `AlertsController` serves list / acknowledge and `AlertsPage` (M1-10) renders them. Nothing alerts off a provisional baseline, a paused member, or a condition already open. Notes/photos and alert-detail screens (M1-11/12/16) not started; no push delivery ([notification_engine.md](./technical/notification_engine.md)) |

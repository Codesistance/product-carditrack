...
| **CardiJournal — the Weekbook** | R2 | R2 | — | Complete Care | 🔶 **Backend shipped in dev; mobile not started.** `DigestAudience.Weekbook` + `IX_DigestEntries_OneWeekbookPerWeek` (its own partial index — a Weekbook and a Daybook share a `LocalDate` most weeks, so one index over both audiences would refuse the second write), `GenerateDueWeekbooksAsync` riding the existing half-hourly `--job digest` pass, `WeekbookPrompt`, and `?audience=weekbook` on the insights digest endpoints. Due on the member's own `JournalWeekStartsOn` once their `WeekbookLocalTime` passes, dated by the week's **last day**, written once. **Guard: at least 4 of 7 days must carry readings** — an unmeasured week gets no account, because one would have to speak for the days that are missing. Built from the week's own measurements and never from its Daybooks, so no imprecision propagates upward and a week whose Daybooks were discarded still gets its Weekbook. Register guards are now shared (`JournalRegisterGuards`). *Remaining: the mobile Weeks segment and entry page, and the plan gate (separate effort — nothing is gated today).* ~~Not started — but sold.~~ The pricing page lists a weekly round-up under Complete Care as of 2026-08-18. Only the daily `DigestAudience.Daybook` exists (own row above, prod-gated). The Weekbook needs its own audience value, its own due rule and its own prompt — a week is not a longer day, and the daily prompt's register does not carry over unexamined. **It is a raw reassessment of the week from that week's measurements, not a summary of the seven Daybooks** — so it still gets written for a week whose Daybooks were skipped or discarded |

...
**AI:**
- MedGemma 1.5 4B (`medgemma-1.5-4b-it:q4_k_m`, built from vendored Q4_K_M weights) served via **Ollama on Cloud Run** — the Medical provider for health-data interpretation
...
| Gemini via Vertex AI | BAA-eligible under the Cloud umbrella — **D6 resolved 2026-08-21**: the Public and Rewrite slots move to an EU regional Vertex endpoint (in code; per-environment flip pending) (ZDR configuration, [vertex_ai_setup.md](technical/vertex_ai_setup.md)); the consumer Gemini API is superseded. Chat and reports still send **de-identified daily readings** (no name/id on chat; reports label members positionally). **Every surface a caregiver reads is now two-step** — the family digest and Advise since 2026-09-04 / 2026-08-26, the dashboard status line since 2026-09-15, and the insight cards, the trend narrative, the CardiJournal books, the statistical alert and the real-time heart-rate alert since 2026-09-23. In every one of them MedGemma's clinical read stays in-project and only a de-identified read reaches the Vertex Rewrite slot (DPIA A11, A13, A15, A22, A24). What stays on in-project MedGemma alone is the work no family reads directly: the real-time assessment itself, whose other four consumers are prompts |

...
- **Header names the member and the book** — "Dad's Daybook", "Dad's Weekbook": the list passes the first name so the header is right from the first frame; a deep link without one falls back to "Daybook" until the member fetch fills it in. The line beneath names the period: "One day / week / month, read against what is usual for them"
- **"The 14 days up to …"** on a Daybook, **"This week and the one before, to …"** on a Weekbook, **"The 30 days up to …"** on a Monthbook — trend charts for Sleep, Resting heart rate, Blood oxygen, Breathing rate, Steps and Skin temperature, drawn with the same `TrendChart` the alert detail uses: the member's own usual dashed, the published band shaded, per-day markers. Skin temperature carries only the wearer's own nightly baseline (no published band, no counted line); Steps carries only the usual (no body publishes a daily step count)
...
- **Up to two counted lines under each chart** (`TrendAwareness`), over the same window the chart draws (14 days, or 30 on a Monthbook): against the member's own usual — "Under their usual on 10 of the last 14 nights" — and against the published band with its publisher named — "Under the recommended 7h (NSF) on 12 of the last 13 nights" (`BandLine`). Counts, never scores, per the release matrix's standing no-risk-scores decision; every bound comes from `HealthReferenceRanges` so the sentence can never cite a figure the chart does not shade; partial and unmeasured days are on neither side; nothing is said below 7 measured days
- The charts are the window **ending on the entry's own day**, whatever the calendar has done since, and the section title names that day. The series is fetched ending on that day (`?seriesEndsOn=`), because the live profile's series always runs to today and a Monthbook's month — or a Weekbook a fortnight old — lies entirely outside it. A series that still falls short of the window hides the charts rather than drawing days the entry is not about
...
- **Edge — charts fetch fails:** the review stands and the trends section hides; a loaded review must not be replaced by an error panel over its garnish. **Edge — no review for the date:** "No review was written for this day". **Offline:** the live profile the device holds (the dated series is never cached — its key is one the profile's own invalidation cannot spell), whose reach the window check honours
- Backed by `GET .../digest?date=YYYY-MM-DD&audience=daybook|weekbook|monthbook` + `GET /api/v1/cardimembers/{id}?seriesEndsOn=YYYY-MM-DD` for the chart series. On a Weekbook the date is the week's **last day**, so the 14-day window renders as this week against the one before it — the comparison a week's account wants, at no extra cost. A **Monthbook draws 30 days**, and the counted lines move to the same window: the sentence under a chart claims to describe it, so the two are one number by construction
...
Datadog telemetry ships **logs, traces and RUM** (views, errors, crash reports — no action or resource tracking); crashes and ANRs also come from **Play Console vitals**. Tracking is **on by default with an always-available opt-out** (since 2026-09-24; disclosed in the Terms of Service and Privacy Policy) — nothing is sent before sign-in, then the SDK is `TrackingConsent.Granted` and Settings → Privacy → **Send session telemetry** turns it off (`NotGranted`), applying without a restart; sign-out forgets the choice and stops collection until the next sign-in. Beneath it, **Share app logs** zips the on-device Serilog files and opens the share sheet; it is deliberately not gated on the toggle, which governs what the app sends unprompted rather than what the caregiver hands over themselves. No separate diagnostics screen exists, and a one-time **"About app diagnostics"** modal on the dashboard tells each caregiver what is sent and where to turn it off (**Got it** / **Open Settings**; seen per caregiver, cleared on sign-out — Story 7.4, `TelemetryNotice`).
...
> **As built today (MVP 1 `SettingsPage`):** an account card, a **"Silenced reminders"** card listing every held notification mute with a "Show me everything again" reset, then four grouped section cards — **Notifications** (quiet hours and what to hear about), **Account** (change password, export health data, export confirmations), **Privacy** (send session telemetry, on by default with an opt-out; share app logs), **About** (terms, privacy policy, version) — then Sign out and the account-deletion request card. The groups follow Journal Settings’ shape: a section card naming the group with one line on what it is for, and inside it an outlined card of rows parted by hairlines, each row carrying its control on the right (a chevron, a switch, or a value). The placeholder card is gone. The grouped list below is still the MVP 2 design intent — subscription, family sharing and profile editing are not built.
...
**Notifications**
- Alert Settings →
- Custom Alerts → *(R2 — caregiver-defined thresholds; reached from Member Details' Management group, needs design sync)*

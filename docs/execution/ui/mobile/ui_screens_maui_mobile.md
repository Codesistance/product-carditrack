...
- **Edge — charts fetch fails:** the review stands and the trends section hides; a loaded review must not be replaced by an error panel over its garnish. **Edge — no review for the date:** "No review was written for this day". **Offline:** the series the entry was last drawn from, or failing that the live profile the device holds, whose reach the window check honours

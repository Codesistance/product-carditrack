...
- Visual progress bar (today vs the previous calendar day's total; the track's max is yesterday until today exceeds it, then it is today's total — one fill, no overflow colour). Caption is a comparison ("vs 5,000 yesterday"), not a remainder against a goal. Hidden when day n−1 is missing.

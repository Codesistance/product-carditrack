...
- **Current state (shipped):** Datadog telemetry is **logs, traces and RUM** (views, errors, crash reports — no action or resource tracking); crashes/ANRs also come from Play Console vitals. Since 2026-09-24 it is **on by default with an always-available opt-out** (opt-in 2026-09-14 → 2026-09-24): nothing is sent before sign-in, then the SDK is `TrackingConsent.Granted`, it is disclosed in the Terms of Service and Privacy Policy, and Settings → Privacy → **Send session telemetry** drops it to `NotGranted` at once. The mobile payload carries no name or email, but its traces join API traces tagged with the pseudonymous account ID, so it can be matched to an account when investigating — and the disclosures say so.

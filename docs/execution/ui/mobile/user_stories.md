...
> **Build status (September 18, 2026):** all 17 Figma M1 screens are built in `CardiTrack.Mobile` (M1-01 through M1-17). Alert detail is one page (`AlertDetailPage`) covering M1-11/12/16, charted from the single series that caused the alert. **Ten** shipped surfaces have **no Figma M1 frame — needs design sync**: SignInPage, ForgotPasswordPage, VerifyEmailPage, Onboarding/AccountSetupPage (see Stories 1.5–1.8), plus NotificationsPage, JournalPage, JournalEntryPage, ExportConsentsPage, QuestionCard and the Questions & Answers page — the canonical list is in [ui_screens_maui_mobile.md](./ui_screens_maui_mobile.md) Build Status. Release waves re-baselined: MVP 1 (R1) → Q4 2026, MVP 2 (R2) → Q1 2027, MVP 3 (R3) → Q2 2027. Release sequencing is governed by the [release matrix](../../../release_matrix.md).
...
- **Current state (shipped):** Datadog telemetry is **logs, traces and RUM** (views, errors, crash reports — no action or resource tracking); crashes/ANRs also come from Play Console vitals. Since 2026-09-24 it is **on by default with an always-available opt-out** (opt-in 2026-09-14 → 2026-09-24): nothing is sent before sign-in, then the SDK is `TrackingConsent.Granted`, it is disclosed in the Terms of Service and Privacy Policy, and Settings → Privacy → **Send session telemetry** drops it to `NotGranted` at once. The mobile payload carries no name or email, but `FirstPartyHosts` would join its traces to API traces tagged with the pseudonymous account ID if trace headers were sent (latent while resource tracking is off), so the disclosures say it may be possible to match it to an account.
...
- [x] Story 7.4: Telemetry consent (off before sign-in, then on by default with a Settings opt-out; one-time dashboard notice shipped)

...
**Basic Info:** "First Name*", "Last Name" (optional), "Date of Birth*" (picker), "Relationship*" (picker)
...
- **M1-17c — Complete:** Success message with Save, Share and Open actions
...
- **Two delivery actions — Save and Share — with Open beneath them** (changed 2026-09-19). It was one button reading "Save or share", which opened the native share sheet, plus an **Open** beside it. The reasoning was that the sheet *is* the route to Save to Files / Save to Drive, so a separate Save would open the same thing — true, and it still left one button naming two actions and a caregiver who wanted a copy on their phone needing to know the save was hidden inside a sheet. **Save** now keeps the file where the platform's own file manager reads (Android: Downloads, via MediaStore; iOS: the app's folder in Files) and says where it went; **Share** opens the sheet. **Open** stays, under the two rather than beside them: looking at the file is a real thing to want, and is not a place to put it — it puts the file nowhere, and says so plainly when no installed app claims the type (likelier for a FHIR bundle than for a PDF). The same three, in the same words, are what the export popup offers on the CardiJournal and member-chat export paths — see `Services/ExportFileDelivery.cs` and `Controls/ExportDeliveryPopupPage.xaml`. Save is hidden on any platform with no caregiver-visible folder. "Email to…" is still not built: mailing PHI from the server is its own subsystem (provider, address verification, abuse limits) and is not MVP 1 scope.

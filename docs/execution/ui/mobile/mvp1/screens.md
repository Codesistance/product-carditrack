...
- **Sharing row** — a "SHARING" caption, **one pill per dataset family**, and a chevron. Datasets are mapped from the granted OAuth scopes by `DeviceDatasets` (`CardiTrack.Mobile.Core`), then collapsed by family, so a fully-granted Fitbit reads `Activity 5` · `Heart 2` · `Sleep 2` rather than nine pills. Pills are tinted by family (activity blue, heart rose, sleep purple, body teal, unrecognised grey) in the design's `Activity, HR, Sleep` order.
  ...
  - **Open (issue #82):** `FitbitApiClient` now ingests SpO2, VO2 max, breathing rate and body temperature under the same `health_metrics_and_measurements` scope, but none of them is named — no screen displays them, so naming them would promise readings nothing surfaces. The mapping therefore still under-reports what the connection shares. The card no longer blocks the fix: the four would land inside existing family groups rather than adding four pills to the row.

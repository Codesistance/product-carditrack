...
> **Telemetry consent (shipped):** Datadog telemetry is logs, traces and RUM (views, errors, crash reports — no action or resource tracking); crashes/ANRs also come from Play Console vitals. Since 2026-09-24 it is **on by default with an always-available opt-out** (opt-in before that): the SDK initialises at `TrackingConsent.Granted`, it is disclosed in the Terms of Service and Privacy Policy, and Settings → Privacy → **Send session telemetry** turns it off at once (`NotGranted`). Sign-out forgets the choice and returns to the default. What remains open is an in-app first-run notice — see Story 7.4 in the canonical [user_stories.md](../user_stories.md).

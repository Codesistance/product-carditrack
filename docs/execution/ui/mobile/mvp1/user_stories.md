...
- **Acceptance Criteria:**
  ...
  - The journal list exports every Daybook, Weekbook or Monthbook in the current time window from an icon beside Days / Weeks / Months. Opening an entry exports that one book from an icon in the header. Neither opens M1-17.
...
> **Telemetry consent (shipped):** Datadog telemetry is logs, traces and RUM (views, errors, crash reports — no action or resource tracking); crashes/ANRs also come from Play Console vitals. Since 2026-09-24 it is **on by default with an always-available opt-out** (opt-in before that): the SDK initialises at `TrackingConsent.Granted`, it is disclosed in the Terms of Service and Privacy Policy, and Settings → Privacy → **Send session telemetry** turns it off at once (`NotGranted`). Sign-out forgets the choice and returns to the default. A one-time dashboard notice (`TelemetryNotice`) tells each caregiver what is sent and where to turn it off — see Story 7.4 in the canonical [user_stories.md](../user_stories.md).

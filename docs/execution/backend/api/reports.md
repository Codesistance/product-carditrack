...
| `includeTrends` | boolean | No | Default `true`. On PDF, draws line charts for the selected days (gaps for missing days, never zeros). A pinned journal entry charts the same fortnight (or 30 days for a Monthbook) the journal page does. Counts as a selected section on PDF. Ignored by CSV and FHIR |
...
- **PDF** — the AI narrative (labelled as AI-generated), then age and sex (not date of birth), optional trend charts for the selected days (or, for a pinned journal entry, the same fortnight / 30 days the journal page charts), a daily table per member when metrics are ticked, then alerts, journals (each labelled as AI), and notices. A confidentiality footer and page numbers on every page, because printed pages get separated. A reading the device never reported prints as an em dash, never a zero.

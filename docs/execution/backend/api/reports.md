...
- **No caregiver free text crosses into any export** — no medical notes, no alert message bodies, no caregiver device labels ([data_protection_architecture.md](../../../technical/data_protection_architecture.md) §70, §85). Journals are CardiTrack-generated AI text and may be included on PDF/CSV when ticked; each entry is labelled as AI. Notices export `RuleCode` / category / state, never localized `TitleKey` bodies. **There is deliberately no notes section and no `includeNotes` toggle:** the only free-text notes the system holds are `CardiMember.MedicalNotes`, which this rule excludes from every export. A dead `includeNotes` flag sat on the request until 2026-09-21 — it was never read by any renderer, never counted toward the "at least one section" check, and never entered the consent fingerprint — and was removed rather than wired up.
...
| `includeTrends` | boolean | No | Default `true`. On PDF, draws line charts for the selected days (gaps for missing days, never zeros). A pinned journal entry charts the same fortnight (or 30 days for a Monthbook) the journal page does — the request range stays on the header, daily table, filename and narrative. Counts as a selected section on PDF. Ignored by CSV and FHIR |
...
- **PDF** — the AI narrative (labelled as AI-generated), then age and sex (not date of birth), optional trend charts for the selected days (or, for a pinned journal entry, the same fortnight / 30 days the journal page charts — the header, daily table and narrative stay on the request range), a daily table per member when metrics are ticked, then alerts, journals (each labelled as AI), and notices. A confidentiality footer and page numbers on every page, because printed pages get separated. A reading the device never reported prints as an em dash, never a zero.

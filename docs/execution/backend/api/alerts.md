...
| `GET /api/v1/alerts/{alertId}` | Detail for M1-11/12/16. Same view-access / 404-not-403 rule. Carries **one** chart series chosen from the alert's `rule` (steps for activity/trend/no-morning, resting HR for elevated HR, sleep hours for irregular sleep, granular HR for `realtime_hr`, HRV for `hrv_drop`, overnight breathing for `overnight_breathing_up`, raised-heart-rate minutes for `elevated_zone_without_movement`, longest still stretch in hours for `daytime_inactivity_block`; and for the published-range rules the metric with the range it was judged against as the reference band — sleep hours with the NSF band, resting HR with AHA's 60–100, blood oxygen with WHO's 94–100). `device_silence`, `ecg_afib` and `irregular_rhythm` have no chart — the last two are events the device stamped rather than readings with a trend behind them, and the default series would put a fortnight of step counts under an atrial-fibrillation finding. Does **not** return the dashboard's six-metric payload. |
...
>
> **Third automated producer (AI pipeline, `assess` job):** `StatisticalAlertService` — the R1 statistical pass — evaluates the fourteen-rule taxonomy on every assessor run — the nine comparative rules below against the **established 30-day baseline only**; since 2026-09-22 the two measured rhythm rules (`irregular_rhythm`, `ecg_afib`), which relay a finding the device itself made and are deliberately not gated on a baseline; and since 2026-09-25 the three **published-range** rules (`sleep_outside_range`, `resting_hr_outside_range`, `spo2_below_range`), which compare against a range someone else publishes rather than the member's usual and so are not gated on a baseline either — and hands every finding that survives cooldown and dedup to MedGemma (`CARDITRACK_STATISTICAL_JUDGEMENT_PROMPT`) in one call per member. **The rules are an input provider; the verdict is the model's:** the clinical half returns a severity (`critical`/`high`/`medium`/`low` → red/orange/yellow/nothing raised) and a clinical read per finding, matched to each finding by its `rule`; the reads that clear Yellow then go in one further call to the **Rewrite** slot, which writes the headline and message a caregiver reads from them and nothing else (split 2026-09-22). A verdict the parser cannot map, a missing verdict, a blank clinical read, a model failure on either slot, a rewrite that drops an entry, or copy a register guard rejects raises nothing (fail closed) and is re-judged next pass. *Moved out of the Worker on 2026-09-19: until then each rule carried its own hard-coded severity and copy straight into the alert row, so a threshold constant paged families with no model in the loop.* Cooldown and dedup semantics are in the note under the taxonomy table.
...
**Per-CardiMember alert-rule enablement is shipped.** `GET /api/v1/cardimembers/{id}/alert-preferences` returns clustered rules with effective on/off state (missing preference row = all on). `PATCH .../alert-preferences/rules/{ruleId}` toggles one rule immediately (**409** when another device or a chat-confirmed change wrote the preference row between this request's read and save — the row carries Postgres's row version as its concurrency token, 2026-09-17). Off means the producer **skips evaluation entirely** for that rule — no `Alert` row. Primary caregiver only for writes; any viewer of the member may read. Future A–G rules appear in the catalogue with `isImplemented: false` until their producers land.

**Both the rule switches and the member's alarms can be changed from the member chat (2026-09-17).** "Turn off the activity decline alert", "alert me if his heart rate goes over 120", "which alerts are on for Dad?" route to the chat's `settings` workflow, which proposes the one change it understood in the same words this API uses (the catalogue titles; `condition` as `MetricAlarmNarrative` composes it) and applies it on the caregiver's yes in the next message — through `IAlertPreferenceService.SetRuleEnabledAsync` and the member-scoped `IMetricAlarmService` methods above, so the same primary-caregiver rule and the same validation hold, and the settings pages and the chat cannot describe a different state. Account-level defaults, quiet hours and muting are not changed from chat (the last two are redirected to the notification preferences). A chat reply that applied a change carries `changedAlertSettings: true` and is audited as `ChangeAlertSettingsViaChat`. Design: [member_chat_routing.md](../../../technical/member_chat_routing.md) §5, `settings`.
...
| Type (`rule`) | Severity (the rule's former constant; since 2026-09-19 the model's verdict) | Implemented enum | Status | Description |
|------|---------------|------------------|--------|-------------|
| `activity_decline` | yellow | `Inactivity` (1) | **Built** (`StatisticalAlertService`, assessor job) | Yesterday's steps >30% below the established 30-day baseline average |
| `elevated_heart_rate` | orange | `HeartRate` (2) | **Built** (`StatisticalAlertService`, assessor job) | Yesterday's resting HR above baseline avg + max(2σ, 5 bpm) |
| `no_morning_activity` | red | `PatternBreak` (4) | **Built** (`StatisticalAlertService`, assessor job) | A **measured zero** steps today past typical wake time + 2 h grace, while the device is reporting — a null steps value (HR-only device) never fires |
| `irregular_sleep` | yellow | `Sleep` (3) | **Built** (`StatisticalAlertService`, assessor job) | Last night's sleep >30% off baseline average, either direction. Sleep sessions are attributed to the civil day they **ended** on, so last night is **today's** activity row — the same row the dashboard's sleep card rates — with yesterday's row as the fallback for a night whose data arrived after local midnight. The alert stamps the `night` it judged into `MetricValues` and dedups per night (not per firing day), so a late-synced night still alerts exactly once. **The trigger is symmetric; what it alerts on is not** — see the note below |
| `device_silence` | yellow | `Inactivity` (1) | **Built** (`InactivityDetectionWorker`) | No granular readings for >2 h during local waking hours (the scenario the design called `device_disconnected`). **Not raised when a device grant has been refused** (`TokenExpired`/`AuthError`): the caregiver gets the Safety nudge `DEVICE_AUTH_BROKEN` ("needs reconnecting") at that moment instead, and a standing silence alert is resolved — see `notification_engine.md` §3 |
| `long_term_trend` | orange | `Trend` (5) | **Built** (`StatisticalAlertService`, assessor job) | Weekly step average declining ≥5%/week for 4 consecutive weeks (each week needs ≥4 measured days) |
| `hrv_drop` | orange | `HeartRate` (2) | **Built** (`StatisticalAlertService`, assessor job) | Overnight HRV (RMSSD) below baseline − max(2σ, 15% of baseline) on **both** of the last two nights. Two nights, because a single low night is a late meal or a glass of wine in someone with nothing wrong; the floor is proportional rather than absolute because RMSSD is not comparable between people |
| `overnight_breathing_up` | orange | `PatternBreak` (4) | **Built** (`StatisticalAlertService`, assessor job) | Last night's *asleep* breathing rate above baseline + max(2σ, 1 breath/min). The overnight figure, not the daily one — a whole-day average moves with stairs and naps. Baseline-relative rather than band-relative: WHO's 12–20 is wide enough to hide a four-breath rise inside it, and the band is quoted in the copy for context only |
| `elevated_zone_without_movement` | orange | `HeartRate` (2) | **Built** (`StatisticalAlertService`, assessor job) | Minutes above the light heart-rate zone past max(their usual, 25 min) on a day `activity_decline` also considers quiet. The pairing is the finding: the same minutes after a walk are exercise, and on a 1,200-step day they are a heart working without being asked to |
| `sleep_outside_range` | *model* | `Sleep` (3) | **Built** (`StatisticalAlertService`, assessor job) | Sleep outside the NSF range for the member's age on ≥3 of the last 5 nights with a reading, **whatever their own usual is** (decision 2026-09-25). A **published-range** rule: no baseline needed. Awake nights count as 0 h; no-data and pending nights are skipped. With no age only the 7 h floor is judged. One alert per stretch: an alert of the same rule raised since the stretch began (a stretch ends after 3 measured nights back inside the range) keeps it quiet, resolved or not. The finding states three weekly averages and whether each sits further outside the range than the last; the model weighs a worsening stretch above a steady one |
| `resting_hr_outside_range` | *model* | `HeartRate` (2) | **Built** (`StatisticalAlertService`, assessor job) | Resting heart rate outside AHA's 60–100 bpm, either side, on ≥3 of the last 5 days, judged through yesterday. Same stretch, trend and no-baseline semantics as `sleep_outside_range`; shares the heart-rate cooldown with the other `HeartRate` rules |
| `spo2_below_range` | *model* | `PatternBreak` (4) | **Built** (`StatisticalAlertService`, assessor job) | Blood oxygen below WHO's 94% on ≥3 of the last 5 days. Same stretch, trend and no-baseline semantics; below only, since the range tops out at 100 |
| `daytime_inactivity_block` | yellow | `Inactivity` (1) | **Built** (`StatisticalAlertService`, assessor job) | One unbroken sedentary stretch past max(3 h, their usual + 50%). Reads `activity-level` as *intervals*: the day's sedentary total cannot tell six hours in half-hours from six hours at once, and only the second is worth a word. Only time the watch was worn counts: the intervals are clipped to five-minute windows with a heart rate, so a watch on its charger is never reported as a wearer sitting still, and a day whose heart rate cannot be read reports no stretch |
| `ecg_afib` | *model* | `Rhythm` (6) | **Built** (`StatisticalAlertService`, assessor job) — inert until issue #39's verification covers `googlehealth.ecg.readonly` | An ECG the wearer recorded on their own device came back classified `ATRIAL_FIBRILLATION`. A **measured** rule: it reports what the device found, so it takes no baseline and is not gated on one. Only that one classification counts — `INCONCLUSIVE*`, `UNREADABLE` and `NOT_ANALYZED` all mean the device declined to judge, and the ingestion layer never counts them. Unlike `irregular_rhythm` there is a trace behind this a clinician can open, and the observation says so |
| `irregular_rhythm` | *model* | `Rhythm` (6) | **Built** (`StatisticalAlertService`, assessor job) — inert until issue #39's verification covers `googlehealth.irn.readonly` | The watch's own optical screening raised one or more irregular-rhythm notifications. Measured, like `ecg_afib`, and takes no baseline for the same reason. Counted per **notification**, not per `alertWindow` — a notification is one thing the wearer was told, and reporting the windows behind it would inflate one alert into several |

> **Cooldown scope follows the family's remedy.** Every producer stamps its `rule` into `MetricValues`, and one *unresolved* alert per rule suppresses that rule (resolving re-arms it) — except `HeartRate`, which is deliberately **type-scoped across producers**: an unresolved heart alert from either the AI assessor (`realtime_hr`) or the statistical rule suppresses the other, because the remedy is the same ("check on them") and two simultaneous heart pages about one person is the noise cooldowns exist to prevent. Caregiver-defined alarms (`custom:{alarmId}`) sit **outside** the heart cooldown in both directions: their alert re-arms through the alarm's own state row and is never resolved by a producer, so it neither suppresses the assessor and the statistical heart rules nor is closed by the assessor's ordinary-hour pass — it suppresses only its own rule. `Inactivity`'s two rules (`device_silence`, `activity_decline`) ask for different remedies — charge the watch vs. encourage movement — and may stand together. Daily-grain statistical rules additionally dedup per local day: resolving **or deleting** at noon does not re-page at half past from the same readings (`irregular_sleep` dedups per **night judged** rather than per firing day — see its row). Soft-delete is housekeeping, not a new episode: the statistical engine still sees the deleted row for that same-day/same-night check (but not as a forever latch — it does not auto-resolve, so a deleted alert must not silence the rule on later days), while `device_silence` and `realtime_hr` treat a deleted unresolved alert as the same episode until their own "the condition has passed" resolve fires. **Provisional baselines never fire a *comparative* rule** — one that asks whether a reading is unusual for this member. The engine fetches only the established 30-day baseline, so without one every such rule is silent by construction. The two **measured** rules (`ecg_afib`, `irregular_rhythm`) are deliberately outside that gate: they report a finding the wearer's own device made and classified, so there is no inference in them for a thin window to weaken, and suppressing them would mean a member two weeks into wearing a watch hears nothing when it tells them their heart is in atrial fibrillation. Without a baseline the activity-log read narrows to two days, since only those two rules can run.
...
> Severities use the product taxonomy (`yellow`/`orange`/`red`), plus `green`, which now reaches a caregiver only from the **AI assessor** (`Low` maps there). The statistical pass never writes `green`: a `low` verdict on a statistical finding means the model judged it not worth attention today, and nothing is raised. Since 2026-09-23 that verdict is **remembered** in `BenignJudgements` — a row of its own rather than a green alert, because a green alert is a card a caregiver can open. It is keyed on a hash of the finding itself (`StatisticalAlertRules.JudgementFingerprint`: the rule, the observation and the figures behind it) rather than on the day, so a verdict stands only for as long as the readings it was reached on do — a repair sync or a late-arriving night changes the fingerprint and the finding is judged again. That covers every rule without a list of exceptions: `no_morning_activity` names the clock in its observation and so re-judges every pass, and the two measured rules carry the device's own counts, so a notification arriving this afternoon is a new question on its own. Rows are swept after a week and erased with the member. `irregular_sleep`'s former benign branch was retired to the daybook entry. Historic `green` rows remain on file and still render, so clients must keep handling the value. `green` is *not* emitted for merely normal states: no alert is raised when nothing departed from baseline at all. **A `green` alert is a full alert everywhere except the status tier.** Delivery is unchanged by the split — `DeliveryPlanner` already routes both `yellow` and `green` Health rows to in-app + digest and pushes neither. It still forces a digest refresh when newly raised (`DigestGenerationService.AlertStateChangedSinceAsync` gates on `TriggeredDate`, not on severity) and still reaches the digest prompt while unresolved (`MonitoringContextSource` applies its `Yellow` floor to real-time *assessments*; alerts are carried on `!IsResolved` alone). The one thing it does not do is raise the member's dashboard status tier: `ComputeHealthStatus` only takes a label from the worst unresolved alert at `Yellow` and above, so a lone `green` alert leaves the tier where it would have been. Treat "green" as *we looked and it was fine*, not as *we said nothing*. The AI pipeline's internal Critical/High/Medium/Low scale maps to these values — see [llm_design.md](../../../llm_design.md).
...
`reason` is a coarse key for the detail screen's icon — `activity`, `heart`, `sleep`, `device` or `monitoring` — derived from `rule`, falling back to `AlertType` for rows written before rule markers. `hrv_drop`, `elevated_zone_without_movement`, `ecg_afib` and `irregular_rhythm` take `heart`, `daytime_inactivity_block` takes `activity`, `sleep_outside_range` takes `sleep`, `resting_hr_outside_range` takes `heart`, and `overnight_breathing_up` and `spo2_below_range` take `monitoring` — the icons are hand-authored and there is no lungs artwork, so filing a breathing finding under the heart icon would name the wrong organ at a glance. It is deliberately *not* the severity: the banner already carries that in colour, so an icon repeating it says nothing new. Clients should fall back to `monitoring` for an unrecognised value rather than rendering no icon.
...
| `DELETE /api/v1/alarms/{alarmId}` | **204.** Soft-delete — **409** when the row was changed by another commit since this request read it (2026-09-17). Members who had tuned it **keep their own copy** — a caregiver's tuning for one person is an intention about that person, and removing the shared default is not a retraction of it. |
...
| `PUT /api/v1/cardimembers/{id}/alarms/{alarmId}` | Given an account default's id, writes this member's override of it; given a member alarm's own id, edits that row. Same state-reset rule as the account-level PUT. An override saved back to **exactly what the account default says** — or a disabled override switched back on with nothing else changed, which is what the list page's toggle sends — is removed rather than kept, and the default applies again: a detached copy marked "tuned for them" that silently stops following account edits is not what the caregiver asked for. **409** when two devices write the same first override at once, or when the row was changed by another commit between this save's read and its write — `MetricAlarms` carries Postgres's row version as its concurrency token (2026-09-17); the other save stood. |
| `DELETE /api/v1/cardimembers/{id}/alarms/{alarmId}` | **204.** Reverts an override to the account default, or deletes a member-only alarm; **409** when the row was changed by another commit since this request read it (2026-09-17). Accepts either identity so a client holding one list row need not know which it has. |

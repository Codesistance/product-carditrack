...
| `elevated_heart_rate` | **yellow** / orange | `HeartRate` (2) | **Built** (`StatisticalAlertWorker`) | Yesterday's resting HR above baseline avg + max(2σ, 5 bpm). **The trigger is member-relative; the severity is not** — `orange` only once the reading has also left the published adult band — see the note below |
...
> **`irregular_sleep` grades by direction, against the published band.** A departure from the member's own usual cannot say on its own whether a night was a problem, because the usual it is measured against may itself be far short of what anyone should be getting: someone who normally manages 3.8 h and slept 5.2 h is 37% off their baseline *and* closer to the recommendation than they have been all fortnight. Grading that the same amber as a night that collapsed to 2.4 h asks a caregiver to worry about an improvement. So a **longer** night is `green` — informational, still on the list, but not dressed as a warning — until it passes the NSF ceiling for the member's age (`HealthReferenceRanges.Sleep`: 7–9 h, 7–8 h from 65), which is the one direction in which more sleep is the reading worth flagging. A **shorter** night keeps `yellow` whatever the absolute figure, because losing a third of someone's sleep overnight is a pattern break in its own right. The band the night was judged against is stamped into `MetricValues` (`recommendedLowHours` / `recommendedHighHours`) so a member who later crosses 65 cannot get copy quoting one ceiling beside a chart shading another.

> **`elevated_heart_rate` grades against the published band too.** Same reasoning as `irregular_sleep`, applied to the other metric that has a published adult range. The margin is this member's own variability, so clearing it says a pattern broke — not how much of a break it is: a member who normally rests at 52 and read 60 has cleared `max(2σ, 5)` and landed in the middle of the AHA's 60–100, while a member who normally rests at 96 and read 103 cleared the same margin and left it. Grading both `orange` — a push, and a CHECK IN on the card — spends a caregiver's attention on an ordinary resting heart rate, which is how they learn to ignore the one that is not. So `orange` needs the reading to have passed the AHA ceiling as well; every other break is `yellow`, still raised and still on the list, because a sudden upward shift is a pattern break whatever the absolute figure. Note the band can be cleared **downwards** — a member whose usual is 45 reading 52 broke their pattern upward while sitting under the published floor throughout — and the copy says so rather than rounding it to "running high". The band is stamped into `MetricValues` (`typicalLowBpm` / `typicalHighBpm`) for the reason `irregular_sleep` stamps its own: the copy and the chart beside it must not quote two different ranges if the published figures ever move.

> **The digest names the band as well as the usual.** `DigestInterpretationSignals.RaisedVitals` appends where a resting HR landed against the AHA range, and — the case member-relative logic cannot reach at all — emits a finding for a rate that sits **above** the band on a day it matched the member's own usual, which departs from nothing and would otherwise never be mentioned. One-sided on purpose: the AHA floor of 60 is a settled normal for anyone fit or on a rate-controlling medication, and a digest that said so every day is one nobody reads on the day it says something else.

> Severities use the product taxonomy (`yellow`/`orange`/`red`), plus `green` where a rule can positively establish that a departure was benign — `irregular_sleep` is the only statistical rule that can, and the AI assessor's `Low` maps there too. `green` is *not* emitted for merely normal states: no alert is raised when nothing departed from baseline at all. **A `green` alert is a full alert everywhere except the status tier.** Delivery is unchanged by the split — `DeliveryPlanner` already routes both `yellow` and `green` Health rows to in-app + digest and pushes neither. It still forces a digest refresh when newly raised (`DigestGenerationService.AlertStateChangedSinceAsync` gates on `TriggeredDate`, not on severity) and still reaches the digest prompt while unresolved (`MonitoringContextSource` applies its `Yellow` floor to real-time *assessments*; alerts are carried on `!IsResolved` alone). The one thing it does not do is raise the member's dashboard status tier: `ComputeHealthStatus` only takes a label from the worst unresolved alert at `Yellow` and above, so a lone `green` alert leaves the tier where it would have been. Treat "green" as *we looked and it was fine*, not as *we said nothing*. The AI pipeline's internal Critical/High/Medium/Low scale maps to these values — see [llm_design.md](../../../llm_design.md).

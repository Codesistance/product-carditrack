...
| `GET /api/v1/alerts/{alertId}` | Detail for M1-11/12/16. Same view-access / 404-not-403 rule. Carries **one** chart series chosen from the alert's `rule` (steps for activity/trend/no-morning, resting HR for elevated HR, sleep hours for irregular sleep, granular HR for `realtime_hr`, HRV for `hrv_drop`, weight for `rapid_weight_gain`, the day's lowest blood sugar for `blood_sugar_out_of_range`). `device_silence` and both rhythm rules have no chart — a rhythm finding is an event rather than a series, and the only series that could be drawn beside it (resting HR) is routinely unremarkable during atrial fibrillation, so a flat line under it would read as reassurance the reading does not support. Does **not** return the dashboard's six-metric payload. |
...
| 5 | `Trend` | Multi-week decline trend, and rapid weight gain |
| 6 | `Rhythm` | A rhythm finding the device itself made — an irregular-rhythm notification, or an ECG it classified as atrial fibrillation. Deliberately not `HeartRate`: that type is cooldown-scoped as a whole, so a standing "resting heart rate is running high" would have swallowed it |
...
>
> **Third automated producer (Worker):** `StatisticalAlertWorker` — the R1 statistical engine — evaluates the rule taxonomy below every 15 minutes. Its rules split in two: **comparative** rules judge a reading against the member's **established 30-day baseline** and stay silent without one, while **measured** rules report a finding the device itself made (ECG classification, rhythm notification, weighing, glucose reading) and are not gated on a baseline — there is no inference in them to be thin. Cooldown and dedup semantics are in the note under the taxonomy table.
...
| `ecg_afib` | red | `Rhythm` (6) | **Built** (`StatisticalAlertWorker`) | An ECG the wearer recorded came back classified `ATRIAL_FIBRILLATION` by their device. Only that classification counts — the inconclusive and unreadable members mean the device declined to judge. Needs no baseline |
| `irregular_rhythm` | orange | `Rhythm` (6) | **Built** (`StatisticalAlertWorker`) | The device raised one or more irregular-rhythm notifications. Orange rather than red: this is a screening notification from an optical sensor, and the reading a doctor can read is the ECG. Needs no baseline |
| `rapid_weight_gain` | orange | `Trend` (5) | **Built** (`StatisticalAlertWorker`) | Weight up ≥1.4 kg inside 3 days or ≥2.3 kg inside 7, measured against the **earliest** weighing in that window (the lowest would turn one light weighing into a permanent low-water mark). A fluid-retention signal, not a diet one. Needs no baseline |
| `blood_sugar_out_of_range` | red / orange | `PatternBreak` (4) | **Built** (`StatisticalAlertWorker`) | The day's lowest reading below 70 mg/dL (**red**) or its highest above 250 mg/dL (**orange**). The asymmetry is deliberate: a high reading is an afternoon to manage, a low one can take someone off their feet within the hour. Thresholds sit well outside the 70–180 target band the dashboard draws, so ordinary days do not page. Needs no baseline |

> **Cooldown scope follows the family's remedy.** Every producer stamps its `rule` into `MetricValues`, and one *unresolved* alert per rule suppresses that rule (resolving re-arms it) — except `HeartRate` and `Rhythm`, which are deliberately **type-scoped across producers**: an unresolved heart alert from either the AI assessor (`realtime_hr`) or the statistical rule suppresses the other, because the remedy is the same ("check on them") and two simultaneous heart pages about one person is the noise cooldowns exist to prevent. `Inactivity`'s two rules (`device_silence`, `activity_decline`) ask for different remedies — charge the watch vs. encourage movement — and may stand together. Daily-grain statistical rules additionally dedup per local day: resolving **or deleting** at noon does not re-page at half past from the same readings (`irregular_sleep` dedups per **night judged** rather than per firing day — see its row). Soft-delete is housekeeping, not a new episode: the statistical engine still sees the deleted row for that same-day/same-night check (but not as a forever latch — it does not auto-resolve, so a deleted alert must not silence the rule on later days), while `device_silence` and `realtime_hr` treat a deleted unresolved alert as the same episode until their own "the condition has passed" resolve fires. **Provisional baselines never fire a *comparative* rule** — those are skipped wholesale without an established 30-day baseline. The *measured* rules (`ecg_afib`, `irregular_rhythm`, `rapid_weight_gain`, `blood_sugar_out_of_range`) do fire for a member still being learned: gating them would mean a member two weeks into wearing a watch gets no word when it tells them their heart is in atrial fibrillation, and nothing in those findings is an inference about the member at all.
...
`reason` is a coarse key for the detail screen's icon — `activity`, `heart`, `sleep`, `device` or `monitoring` — derived from `rule`, falling back to `AlertType` for rows written before rule markers. The rhythm and HRV rules take `heart`; weight and blood sugar take `monitoring`, because the reason picks a hand-authored icon and inventing an id with no artwork behind it would render as the monitoring icon anyway, minus the honesty. It is deliberately *not* the severity: the banner already carries that in colour, so an icon repeating it says nothing new. Clients should fall back to `monitoring` for an unrecognised value rather than rendering no icon.

...
> Severities use the product taxonomy (`yellow`/`orange`/`red`), plus `green` where a rule can positively establish that a departure was benign — `irregular_sleep` is the only statistical rule that can, and the AI assessor's `Low` maps there too. `green` is *not* emitted for merely normal states: no alert is raised when nothing departed from baseline at all. Delivery is unchanged by the split: `DeliveryPlanner` already routes both `yellow` and `green` Health rows to in-app + digest and never pushes them. What does change is the tiering — a `green` alert does not move the member's dashboard status tier, does not by itself force a digest refresh, and is below the floor `MonitoringContextSource` feeds to the AI prompt. The AI pipeline's internal Critical/High/Medium/Low scale maps to these values — see [llm_design.md](../../../llm_design.md).

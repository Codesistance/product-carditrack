...
| `POST /api/v1/member-chat/members/{cardiMemberId}/messages/stream` | The same send as server-sent events: numbered `step` events as the pipeline runs (plus one `waiting` event of question-specific lines on the long paths), then `answer` (the same response body) and `done`, or `error` with the status and message. Failures before the first step keep their HTTP status. What the app uses since 2026-09-25; see `docs/technical/member_chat_routing.md` "Streaming the send" | — |
...
| `GET /api/v1/insights/members/{id}/trend` | The stored trend narrative — the longer view. `?horizon=rolling\|weekly\|monthly` picks which stretch: `rolling` (the default, and what this returned before the horizons existed) is the daily pass's unaligned read, `weekly` the week ending the evening before the member's own week start, `monthly` the month just gone. Anything else — including a numeric form such as `?horizon=2` — is a **400**, never a silent fallback. Empty text means under a month of readings, which is the learning state, or that nothing fresh enough exists for that horizon (each carries its own staleness ceiling) | [health-data.md](health-data.md) |
...
| `POST /api/v1/cardimembers/{id}/questionnaires` | Volunteer a standing fact (201) | [questionnaires.md](questionnaires.md) |
...
| `PUT /api/v1/questionnaires/{id}/expire` | Retire a question whose day has ended | [questionnaires.md](questionnaires.md) |
...
| [questionnaires.md](questionnaires.md) | Family Questionnaires | **Implemented** — pipeline-generated questions, answered/edited/deleted by caregivers; standing facts volunteered without being asked | — |

...
| `POST /api/v1/member-chat/members/{cardiMemberId}/messages/stream` | The same send as server-sent events: `step` events as the pipeline runs, then `answer` (the same response body) and `done`, or `error` with the status and message. Failures before the first step keep their HTTP status. What the app uses since 2026-09-25; see `docs/technical/member_chat_routing.md` "Streaming the send" | — |

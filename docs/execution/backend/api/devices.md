...
- **Syncing is a 30-minute cron poll** by the Worker (`WearableSyncWorker`), not provider webhooks. Connections belonging to a **removed or monitoring-paused** CardiMember are excluded by `GetDueForSyncAsync`, so a pause genuinely stops collection — see [cardimembers.md](cardimembers.md).

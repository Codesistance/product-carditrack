...
- `goal` on steps is the member's **own baseline average** and **`null` until a baseline exists** — there is no 10 000 fallback, because no standards body publishes a daily step count and a round number would be our arithmetic under nobody's authority. The dashboard Activity bar does **not** fill against `goal`; it compares day n to the previous calendar day's total (max is yesterday until today exceeds it, then the max is today — one fill, no overflow colour). A missing day n−1 hides the bar. `goal` remains the usual-day figure for captions and explainers. `rangeLow`/`rangeHigh` are heart-rate mean ± one standard deviation; `qualityScore` is the 1–5 star rating described below.

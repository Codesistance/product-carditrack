...
- **Client rule for colouring the star row:** where the card shows a status pill, take the pill's colour, so two accents an inch apart cannot disagree. Where it shows none — `steps` and `sleep` as built — take 3–5 `green`, 2 `yellow`, 1 `orange` off the star count. Never colour a star row from `status` on a card with no pill: sleep has a status, and using it is what paints a short sleeper's two stars green.

...
`?audience=` selects which series to read: `family` (the default, and what every caller got before daybook entries existed), `daybook`, or `weekbook` (one entry per finished week, keyed by that week's **last day**). Anything else is **400**, not silently ignored — a typo'd audience returning the family summary would show a caregiver one kind of summary under the heading of another.
...
Digest **history**, newest first. `?audience=family` (the default) returns the current digest and the regenerations behind it, several of which describe the same day; `?audience=daybook` returns **one entry per finished day**, because a review is written once and never recomputed — this is what the mobile **Journal** tab lists. `?audience=weekbook` returns **one entry per finished week**, dated by the week's last day; a week is due on the member's own `JournalWeekStartsOn` and is written from that week's measurements rather than from its Daybooks, so the two series are independent and a member may legitimately have one and not the other for the same date. `?limit=` caps the page (default 24; an omitted or nonsense value takes the default, and the service clamps into range).

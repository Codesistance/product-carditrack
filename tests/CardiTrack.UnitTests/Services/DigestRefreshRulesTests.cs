...
public class DigestRefreshRulesTests
{
    ...
    /// <summary>
    /// A finding the device itself made waives the floor without a baseline in sight — the member
    /// still inside their learning window is exactly the one whose summary would otherwise say
    /// nothing about the ECG their watch flagged this morning.
    /// </summary>
    [Fact]
    public void DeviceReportedAFinding_WaivesTheFloor_ForAnAfibEcg()
    {
        var today = new ActivityLog
        {
        ...
        Assert.True(DigestRefreshRules.DeviceReportedAFinding(today, yesterday: null));
    }

    [Fact]
    public void DeviceReportedAFinding_WaivesTheFloor_ForAReadingOutsideTheSugarThresholds()
    {
        var today = new ActivityLog
        {
            Date = new DateOnly(2026, 8, 10),
            BloodGlucoseMin = 58m,
            BloodGlucoseMax = 140m,
        };

        Assert.True(DigestRefreshRules.DeviceReportedAFinding(today, yesterday: null));
    }

    // An ordinary day is what the floor is for.
    [Fact]
    public void DeviceReportedAFinding_IsFalse_OnADayTheDeviceRaisedNothing()
    {
        var today = new ActivityLog
        {
            Date = new DateOnly(2026, 8, 10),
            ...
            EcgReadings = 1,
            EcgAtrialFibrillationReadings = 0,
            BloodGlucoseMin = 92m,
            BloodGlucoseMax = 155m,
        };

        Assert.False(DigestRefreshRules.DeviceReportedAFinding(today, yesterday: null));
    }
}

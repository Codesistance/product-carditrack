...
public class GeneratedContentRefreshTests
{
    ...
    // Deliberately not tested here: when the page advances its stamps. That rule — record a read
    // only for a member the page ends up showing, and only for the reads the pass actually
    // launched — is CardiMemberDetailPage's own state, and the unit tests reference Mobile.Core
    // rather than the MAUI head. Two tests that claimed to cover it were removed: both passed
    // DateTime.MinValue into IsDue, which is the first test above under another name, and would
    // have gone on passing had the page stamped on intent again.
}

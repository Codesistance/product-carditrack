...
Script paths are relative to this skill directory — from the repo root, prefix them with `.claude/skills/cloud-architect/`. All three are pure stdlib (no `pyyaml` needed — they ship a minimal YAML parser covering a simple subset: nested maps, `- ` lists, scalars). Each takes `--format json` for machine-readable output and `--output <path>` to write to a file instead of stdout.

## References

Load the reference that matches the task — keep this file lean and pull detail on demand:

- **[references/workload-spec.md](references/workload-spec.md)** — the schema all three scripts parse, with the keys each one actually reads and the silent-default traps. Read **before** writing a workload spec or running any script; [references/example-workload.yaml](references/example-workload.yaml) is a verified starting point to copy.
...
Adapted from [borghei/Claude-Skills `engineering/gcp-cloud-architect`](https://github.com/borghei/Claude-Skills/tree/main/engineering/gcp-cloud-architect) v1.1.0, MIT + Commons Clause. Changes from upstream:

- Renamed to `cloud-architect`.
- Dropped the sibling-skill cross-links (`senior-cloud-architect`, `aws-solution-architect`, `azure-cloud-architect`, and the compliance skills) — those skills are not installed here.
- Added `references/workload-spec.md` and `references/example-workload.yaml`, and the CardiTrack context section. Upstream ships no schema or example for the input all three scripts require.

The six upstream reference docs and the three scripts are byte-for-byte unmodified.

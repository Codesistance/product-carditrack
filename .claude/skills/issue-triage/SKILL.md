---
name: issue-triage
description: Triage a CardiTrack GitHub issue through three expert lenses — product-manager (is it worth doing, which wave), software-architect (where the fix lives, which binding rule constrains it), and cloud-architect (only when GCP or infrastructure is in play) — into one verdict, priority and first step, posted back on the issue. Use when asked to triage an issue, groom the backlog, decide "should we do #N", or when a new issue lands and needs a call.
---
...
Three skills already hold that judgement — [`product-manager`](../product-manager/SKILL.md),
...
- **Lenses:** product ✓, architect ✓, cloud ✗ (no GCP surface — a MAUI placeholder string).
...
- Lenses: [product-manager](../product-manager/SKILL.md) · [software-architect](../software-architect/SKILL.md) · [cloud-architect](../cloud-architect/SKILL.md)
